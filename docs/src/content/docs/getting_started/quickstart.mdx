---
title: Grepify *quickstart*
description: >
  Build your first Grepify pipeline in 5 minutes: convert PDFs to Markdown
...
When your source data is updated, or your processing logic is changed (for example, switching parsers or tweaking conversion settings), Grepify performs smart incremental processing that only reprocesses the minimum. And it keeps your Markdown files always up to date.

## Setup

1. Install Grepify (see [Installation](./installation) for other package managers) and the Docling dependency:

    ```bash
    pip install -U grepify docling
    ```

2. Create a new directory for your project:

    ```bash
    mkdir grepify-quickstart
    cd grepify-quickstart
    ```
...
3. Create a `pdf_files/` directory and add your PDF files:
    ...
    mkdir pdf_files
    ```
    You can download sample PDF files from the [git repo](https://github.com/grepify/grepify/tree/main/examples/pdf_to_markdown).

4. Create a `.env` file to configure the database path:

    ```bash
    echo "GREPIFY_DB=./grepify.db" > .env
    ```
...
```
This defines a Grepify App — the top-level runnable unit in Grepify. It binds the main function with its arguments.
...
The power of Grepify is **incremental processing**. Try these:
...
- Browse more [examples](https://github.com/grepify/grepify/tree/main/examples) for real-world patterns (text embedding, RAG, knowledge graphs)
