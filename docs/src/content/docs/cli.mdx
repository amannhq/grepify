...
description: >
  Reference for the `grepify` CLI — app target formats, environment variable
  loading, and subcommands for init, ls, show, update (catch-up or live), and
  drop.
---
Grepify CLI is a standalone tool for easily managing and inspecting your apps.

## Invoke the CLI

Once Grepify is installed, you can invoke the CLI directly using the `grepify` command. Most commands require an `APP_TARGET` argument, which tells the CLI where your app definitions are located.
...
* By default, the `grepify` CLI searches upward from the current directory for a `.env` file.
* You can use `--env-file <path>` to specify one explicitly:

    ```sh
    grepify --env-file path/to/custom.env <COMMAND> ...
    ```
...
Grepify CLI supports the following global options:
...
* `-V, --version`: Show the Grepify version and exit.
...
```bash
grepify drop [OPTIONS] APP_TARGET
```
...
```bash
grepify init [OPTIONS] [PROJECT_NAME]
```
...
```bash
grepify show [OPTIONS] [APP_TARGET] [STABLE_PATH]
```
...
```bash
grepify update [OPTIONS] APP_TARGET
```
...
| `-f, --force` | Skip confirmation prompt. |
| `-q, --quiet` | Avoid printing anything to the standard output, e.g. statistics. |
| `--reset` | Drop existing setup before updating (equivalent to running 'grepify drop' first). |
