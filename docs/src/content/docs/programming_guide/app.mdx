---
title: The Grepify *App*
description: >
  Apps are the top-level runnable unit in Grepify. Learn how to create an App,
...
An **App** is the top-level runnable unit in Grepify.
It names your pipeline and binds a main function with its parameters. When you call `app.update()`, Grepify runs that main function as the root [processing component](./processing_component) which can mount child processing components to do work and declare target states.
...
When you update an App, Grepify:
...
- Each processing component declares a set of target states, and Grepify syncs them as a unit when that component finishes — all writes happen after processing completes, and each target backend applies its batch atomically when supported.

This is why `app.update()` does not "run everything from scratch": Grepify uses the component path tree to decide what can be reused and what must re-run.
...
Grepify needs a database path (`db_path`) to store its internal state. This database tracks target states and memoized results from previous runs, enabling Grepify to compute what changed and apply only the necessary updates.

The simplest way to configure the database path is via the `GREPIFY_DB` environment variable:

```bash
export GREPIFY_DB=./grepify.db
```

With `GREPIFY_DB` set, you can create and run apps without any additional configuration:
...
app.update_blocking()  # Uses GREPIFY_DB for storage
```
...
A **lifespan function** defines the Grepify runtime lifecycle: its setup runs when the runtime starts (automatically before the first `app.update()`), and its cleanup runs when the runtime stops. Use it to configure Grepify settings programmatically or to initialize shared resources that processing components can reuse.

:::tip
If you only need to set the database path, using the `GREPIFY_DB` environment variable is simpler than defining a lifespan function.
:::
...
async def coco_lifespan(builder: coco.EnvironmentBuilder) -> AsyncIterator[None]:
    # Configure Grepify's internal database location (overrides GREPIFY_DB if set)
...
Setting `db_path` in the lifespan takes precedence over the `GREPIFY_DB` environment variable. If neither is provided, Grepify will raise an error.
...
Grepify provides a CLI for managing your apps without writing additional code.
