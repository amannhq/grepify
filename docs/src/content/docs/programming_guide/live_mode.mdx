...
```bash
grepify update --live my_app.py
# or
grepify update -L my_app.py
```
...
Grepify handles change detection, memoization, and target state reconciliation the same way as in catch-up mode.
...
**Handling deletes:** each cycle's declarations are reconciled against the previous run. If a row disappears from the source table between polls, `sync_users` simply doesn't declare it that cycle — Grepify automatically deletes the corresponding target. You don't need to track deletions yourself.
...
For a complete working example, see [`files_transform`](https://github.com/amannhq/grepify/tree/main/examples/files_transform).
