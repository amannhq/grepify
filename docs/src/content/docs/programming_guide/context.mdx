...
---
Grepify provides a **context** mechanism for sharing resources across your pipeline. This is useful for database connections, API clients, configuration objects, or any resource that multiple processing components need to access.
...
Beyond sharing resources, a `ContextKey` also serves as the **stable identity** of the resource it points to. When you anchor sources or targets to a `ContextKey`, Grepify treats *the key itself* — not the underlying value — as the identifier across runs.
...
Pick a `ContextKey` name that reflects the *logical* role of the resource, not its current address. The name is what Grepify persists.

- **Applications**: use any descriptive name — e.g., `"text_embedding_db"`, `"docs_root"`.
- **Libraries**: prefix with your package name and a `/` to avoid collisions with application keys or other libraries — e.g., `"my_library/db"`, `"grepify.connectors.postgres/pool"`.
:::
...
If you need to access context values outside of Grepify processing components — for example, in query/serving logic that shares resources with your indexing pipeline — use `env.get_context()`:
