...
description: >
  The @coco.fn decorator makes Python functions participate in Grepify's change
...
Add a **return type annotation** to memoized functions so Grepify can properly reconstruct cached values. Without a type annotation, cached values may deserialize as basic Python types (`dict`, `list`, etc.) instead of their original types. See [Serialization](./serialization) for details on supported types.
:::
...
- **Cannot mount child components** (`coco.mount(...)` / `coco.use_mount(...)` inside the body). Mounting is a side effect the cache cannot replay; Grepify raises an error if a memoized function attempts it. Either drop `memo=True`, or restructure so the mount happens in a non-memoized caller.
:::
...
- Set the environment variable `GREPIFY_RUN_GPU_IN_SUBPROCESS=1` to run in a subprocess for GPU memory isolation
...
:::note
By default, `coco.GPU` runs functions in-process, so no pickling is required. When using subprocess mode (`GREPIFY_RUN_GPU_IN_SUBPROCESS=1`), the function and all its arguments must be picklable since they are serialized for subprocess execution.
:::
