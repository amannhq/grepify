...
description: >
  A tour of the Grepify Python SDK — package layout (core, connectors,
  resources, ops), common types like StableKey, and how async orchestration
  composes with sync leaf functions.
---
This document provides an overview of the Grepify Python SDK organization and how async and sync APIs work together.

## Package organization

The Grepify SDK is organized into several modules:

### Core package

| Package | Description |
|---------|-------------|
| `grepify` | All core APIs — async by default, sync variants have a `_blocking` suffix |

### Sub-packages

| Package | Description |
|---------|-------------|
| `grepify.connectors` | Connectors for data sources and targets |
| `grepify.resources` | [Common resources](../common_resources/data_types) — shared data types, vector schema annotations, and ID generation utilities |
| `grepify.ops` | Built-in operations for common data processing tasks (e.g., text splitting, embedding with SentenceTransformers) |
...
Grepify's API is **async-first**. The APIs fall into three categories:
...
Grepify provides two ways for async code to call into sync functions:

- **Mounting** — When you mount a processing component, the function is scheduled on Grepify's runtime, not called directly. So an async function can mount a sync processing function.
...
Here `app_main` is async because it uses mounting APIs (`mount_each`), while `process_file` is sync because it only does computation. The sync `process_file` is mounted as a child component — mounting schedules it on Grepify's runtime, so the async parent can mount a sync child without issues.
