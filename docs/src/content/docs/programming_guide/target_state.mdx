...
description: >
  ...
  targets, and rely on Grepify to sync minimal creates, updates, and deletes.
---
:::note[Prerequisite]
...
A **target state** represents what you want to exist in an external system. You *declare* target states in your code; Grepify keeps them in sync with your intent — creating, updating, or removing them as needed.
...
Grepify treats your declarations as the source of truth: if you stop declaring a target state, Grepify will remove it from the target.
:::
...
When your source data changes, Grepify compares the newly declared target states with those from the previous run and applies only the necessary changes.

## Declaring target states

Grepify connectors provide **targets** with `declare_*` methods:
...
## How Grepify syncs target states

Under the hood, Grepify compares your declared target states with the previous run and applies the minimal changes needed — the same create/update/delete sync the [change-scenario diagrams in Core Concepts](./core_concepts#processing-component) walk through, viewed per target state:

<table>
  <thead>
    <tr>
      <th rowspan="2">Target State</th>
      <th colspan="3" style={{textAlign: 'center'}}>Grepify's Action</th>
    </tr>
    <tr>
...
Grepify ensures containers exist before their contents are added, and properly cleans up contents when the container changes.

### What happens when a target's schema changes

When you change a container target state's declaration (e.g., add a column to a table schema, change a primary key), Grepify detects the change and does its best to alter the target in place. If the change is too large to alter (e.g., changing primary keys), the target is dropped and recreated.

When a target is dropped and recreated, Grepify automatically reprocesses all affected components to backfill the data — you don't need to manually trigger `--full-reprocess`. This is handled by the target connector's [child invalidation](../advanced_topics/custom_target_connector#child-invalidation) mechanism, which signals to Grepify whether the change is destructive (all children lost) or lossy (some data may be lost).

## Generic target state APIs

For cases where connector-specific APIs don't cover your needs, Grepify provides generic APIs:
...
These are exported from `grepify` and used internally by connectors. For defining custom targets, see [Custom Target States Connector](../advanced_topics/custom_target_connector).
