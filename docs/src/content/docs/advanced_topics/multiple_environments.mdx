...
description: >
  Run isolated Grepify environments side by side with explicit Environment and
...
---
By default, all Grepify apps share the same environment, which manages a shared database and context. For most use cases, this default behavior is sufficient. However, you can create multiple isolated environments when you need:

- **Library development**: Libraries that depend on Grepify can use their own environment to avoid sharing state with other libraries or the application
...
Apps that don't specify an environment use the default environment (configured via `@coco.lifespan` or the `GREPIFY_DB` environment variable).
...
env1 = coco.Environment(coco.Settings.from_env(db_path=pathlib.Path("./db1/grepify.db")))
env2 = coco.Environment(coco.Settings.from_env(db_path=pathlib.Path("./db2/grepify.db")))
...
env_alpha = coco.Environment(
    coco.Settings.from_env(db_path=pathlib.Path("./alpha/grepify.db")),
    name="alpha"
)
...
def _lifespan(builder: coco.EnvironmentBuilder) -> Iterator[None]:
    builder.settings.db_path = pathlib.Path("./default/grepify.db")
    yield
...
grepify update ./multi_env.py:MyApp@alpha

# Update the app in the default environment
grepify update ./multi_env.py:MyApp@default
```
