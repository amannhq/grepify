...
description: >
  Customize how Grepify fingerprints memoized inputs via __coco_memo_key__ or
...
---
As described in [Function — Change detection](../programming_guide/function#change-detection), Grepify detects [logic, input, and context changes](../programming_guide/function#change-detection) to decide whether a memo can be reused. Function arguments, [`deps`](../programming_guide/function#deps) values, and [context values](../programming_guide/context#change-detection) with `detect_change=True` are all fingerprinted through the same **data fingerprinting** pipeline. By default, most types are fingerprinted automatically. This page covers how to customize that pipeline — how objects are fingerprinted and validated:

- **Memoization keys** — how to control what Grepify uses as the fingerprint for your objects.
...
For each data value (function argument, `deps` value, or context value), Grepify derives a canonical form with this precedence:

1. If the object implements **`__coco_memo_key__()`**, Grepify uses its return value.
2. Otherwise, if you registered a **memo key function** for the object's type, Grepify uses that.
3. Otherwise, Grepify falls back to structural canonicalization for a limited set of primitives/containers.
...
from grepify import register_memo_key_function
...
When Grepify finds a fingerprint match, it calls each state function with the stored state from the previous run:
...
- **`state`** — the current state value. Grepify stores it for the next run.
...
Annotate the `prev_state` parameter with its expected type (matching what you return in `MemoStateOutcome(state=...)`) so Grepify can properly reconstruct stored state values. See [Serialization](../programming_guide/serialization) for details on supported types.
:::
...
A state method can return an `Awaitable`. Grepify handles this automatically:

- **In an async Grepify function**: awaitables from all state methods are gathered concurrently.
- **In a sync Grepify function**: if no event loop is running, Grepify uses `asyncio.run()`. If a loop is already running, it raises an error — switch to an async function or use `@coco.fn.as_async`.
