...
description: >
  Grepify persists target states and memo results in LMDB. Tune map_size and
...
---
Grepify uses an [LMDB](http://www.lmdb.tech/doc/) database to persist its internal state. This database tracks target states and [memoization](../programming_guide/function) results from previous runs, enabling Grepify to detect what changed and apply only the necessary updates.

## Database path

Grepify needs a database path (`db_path`) to know where to store this internal state. The simplest way to set it is via the `GREPIFY_DB` environment variable:
...
```python
settings = coco.Settings(db_path=pathlib.Path("./grepify.db"))
```

Setting `db_path` in the lifespan or `Settings` takes precedence over the `GREPIFY_DB` environment variable. If neither is provided, Grepify will raise an error.
...
|---------|---------|-------------|-------------|
| `max_dbs` | `1024` | `GREPIFY_LMDB_MAX_DBS` | Maximum number of named LMDB databases. Must be &ge; 1. |
| `map_size` | `4294967296` (4 GiB) | `GREPIFY_LMDB_MAP_SIZE` | Maximum size of the LMDB memory map in bytes. Must be &gt; 0; rounded up to the nearest multiple of the system page size. |
...
```bash
export GREPIFY_LMDB_MAP_SIZE=8589934592   # 8 GiB
export GREPIFY_LMDB_MAX_DBS=2048
```
...
settings = coco.Settings(
    db_path=pathlib.Path("./grepify.db"),
