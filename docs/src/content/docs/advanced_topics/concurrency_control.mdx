...
description: >
  ...
  `max_inflight_components` or the `GREPIFY_MAX_INFLIGHT_COMPONENTS`
  environment variable, and understand how Grepify avoids deadlocks in
  nested mount scenarios.
---
Grepify executes each [processing component](../programming_guide/processing_component) as a unit of concurrent work. By default, up to **1024** processing components can be in flight per app. When components perform resource-intensive work (e.g., calling external APIs, running ML models), you may want a tighter limit.
...
You can also set the limit via the `GREPIFY_MAX_INFLIGHT_COMPONENTS` environment variable:

```bash
export GREPIFY_MAX_INFLIGHT_COMPONENTS=4
```
