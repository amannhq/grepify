...
---
The `grepify.ops.entity_resolution` package resolves a set of raw entity names into a deduplicated canonical map. It finds near-duplicate names via embedding similarity (FAISS), then asks a pair-resolver (typically an LLM) to confirm matches and pick the better canonical name.
...
pip install grepify[entity_resolution_llm]

# Core only (for custom resolver implementations)
pip install grepify[entity_resolution]
```
...
```python
from grepify.ops.entity_resolution import ExistingCanonicalPolicy
...
```python
from grepify.ops.entity_resolution import ResolutionEvent
...
```python
from grepify.ops.entity_resolution import PairResolver, PairDecision, CanonicalSide
