...
---
The `grepify.ops.litellm` module provides integration with the [LiteLLM](https://docs.litellm.ai/) library for text embeddings (`LiteLLMEmbedder`) and speech-to-text transcription (`LiteLLMTranscriber`).

```python
from grepify.ops.litellm import LiteLLMEmbedder, LiteLLMTranscriber
```
...
- Implements `VectorSchemaProvider` for seamless integration with Grepify connectors
...
```python
# In a Grepify function, `file` is a FileLike (e.g. localfs.File)
...
You can build a complete pipeline that walks local audio files, transcribes each one, and stores the transcripts in Postgres using the same pattern shown above. See the [examples directory](https://github.com/amannhq/grepify/tree/main/examples) for runnable pipelines.
...
Below are common providers with their model strings and configuration. The `litellm` module is re-exported from `grepify.ops.litellm` for setting provider-specific variables. See the [LiteLLM embedding docs](https://docs.litellm.ai/docs/embedding/supported_embedding) for the full list.
...
```python
from grepify.ops.litellm import LiteLLMEmbedder, litellm
