...
description: >
  Overview of Grepify's built-in operations: text chunking and splitting,
  local and hosted embeddings, audio transcription, and entity resolution,
  with links to each operation module's reference.
---
Grepify ships a set of built-in operations under the `grepify.ops` package.
Each module is independently importable and composes with the rest of a pipeline.

```python
from grepify.ops import text, litellm, sentence_transformers, entity_resolution
```
