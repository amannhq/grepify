...
---
The `grepify.ops.sentence_transformers` module provides integration with the [sentence-transformers](https://www.sbert.net/) library for text embeddings.
...
```bash
pip install grepify[sentence_transformers]
```
...
Here's a complete example of a text embedding pipeline (based on the [text_embedding example](https://github.com/amannhq/grepify/tree/main/examples/text_embedding)):
