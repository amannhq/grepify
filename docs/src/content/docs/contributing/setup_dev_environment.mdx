...
description: >
  ...
  `maturin develop` to get a local Grepify checkout ready for development.
meta:
  time: ~15 minutes
  requires: Rust + uv
---
Follow the steps below to get Grepify built on the latest codebase locally - if you are making changes to Grepify functionality and want to test it out.
...
- Build the library. Run at the root of grepify directory:
...
To run examples during development, you need to use the local editable version of grepify.
...
# Run with your local grepify changes
coco-dev-run grepify update main
```

The `coco-dev-run` function runs `uv run --with-editable $GREPIFY_DEV_ROOT`, which ensures the example uses your locally built grepify package instead of the published version. The `GREPIFY_DEV_ROOT` variable is automatically set to the repo root when you source `.env.lib_debug`.
...
- Run cargo tests via:
  ...
  `./dev/run_cargo_test.sh -p grepify --lib`
