...
---
The `lancedb` connector provides utilities for writing rows to LanceDB tables, with automatic schema inference from Python classes and support for declaring vector and full-text search (FTS) indexes. Grepify manages the table lifecycle — creating, dropping, and evolving the schema — and keeps rows in sync via incremental upserts and deletions.
...
LanceDB connections are created directly via the LanceDB library. Grepify exposes thin wrappers:
...
The `lancedb` connector provides target state APIs for writing rows to tables. Grepify tracks what rows should exist and automatically handles upserts and deletions.
...
:::note
The key name is load-bearing across runs — it's the stable identity Grepify uses to track managed tables. See [ContextKey as stable identity](../programming_guide/context#contextkey-as-stable-identity) before renaming.
:::
...
- `managed_by` — Whether Grepify manages the table lifecycle (`"system"`) or assumes it exists (`"user"`).
  `ManagedBy` is exported from `grepify.connectorkits.target`.
...
Grepify automatically runs LanceDB `table.optimize()` when durable table and index statistics
...
:::note
Automatic optimize runs inline with Grepify's row-mutation path. To avoid LanceDB optimistic
commit conflicts, use Grepify as the owner of writes that sync source data into the table.
External LanceDB connections outside Grepify should generally be read-only while Grepify is
updating the table.
:::
