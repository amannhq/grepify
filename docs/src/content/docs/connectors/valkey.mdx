...
- `client_name` — Client name for the connection, visible in `CLIENT LIST` and monitoring dashboards (default: `"grepify_vector_store"`). Pass `None` to disable.
...
The `valkey` connector provides target state APIs for writing documents to search indexes. Grepify tracks what documents should exist and automatically handles upserts and deletions.
...
:::note
The key name is load-bearing across runs — it's the stable identity Grepify uses to track managed indexes. See [ContextKey as stable identity](../programming_guide/context#contextkey-as-stable-identity) before renaming.
:::
...
```python
from grepify.connectorkits.target import ManagedBy
...
- `managed_by` — `ManagedBy.SYSTEM` (default): Grepify creates and drops the index automatically. `ManagedBy.USER`: assumes the index already exists and never drops it.
