...
The `sqlite` connector provides target state APIs for writing rows to tables. With it, Grepify tracks what rows should exist and automatically handles upserts and deletions.
...
```python
from grepify.connectors.sqlite import Vec0TableDef
