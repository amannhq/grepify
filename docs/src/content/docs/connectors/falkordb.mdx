...
:::note
The key name is load-bearing across runs — it's the stable identity Grepify uses to track managed rows. See [ContextKey as stable identity](../programming_guide/context#contextkey-as-stable-identity) before renaming.
:::
...
The `falkordb` connector provides target state APIs for writing records to node tables and relation tables. Grepify tracks what records should exist and automatically handles upserts and deletions.
...
- `table_schema` — Optional schema definition (see [Table Schema](#table-schema-from-python-class)). FalkorDB does not enforce per-property types server-side, so the schema participates in Grepify's fingerprint (so two flows declaring the same label must agree) but no per-column DDL is emitted.
...
- `managed_by` — Whether Grepify manages the table lifecycle (`"system"`) or assumes it exists (`"user"`).
...
- `managed_by` — Whether Grepify manages the relationship lifecycle (`"system"`) or assumes it exists (`"user"`).
...
```python
from grepify.connectors.falkordb import TableSchema, ColumnDef
...
It then attempts a uniqueness constraint via the `GRAPH.CONSTRAINT CREATE` Redis command (best-effort — failures are logged but do not abort). Indexes and constraints are dropped on `grepify drop` or when the table is no longer declared.
...
async def app_main() -> None:
    ...
    documents.declare_record(
        row=Document(
            ...
            title="Overview",
            summary="An overview of Grepify...",
        )
    )
...
async def kg_app_main() -> None:
    ...
    entities.declare_record(row=Entity(value="Grepify"))
