...
The `qdrant` connector provides target state APIs for writing points to collections. Grepify tracks what points should exist and automatically handles upserts and deletions.
...
:::note
The key name is load-bearing across runs — it's the stable identity Grepify uses to track managed collections. See [ContextKey as stable identity](../programming_guide/context#contextkey-as-stable-identity) before renaming.
:::
...
- `managed_by` — Whether Grepify manages the collection lifecycle (`"system"`) or assumes it exists (`"user"`).
