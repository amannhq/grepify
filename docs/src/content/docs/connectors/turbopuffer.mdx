...
```bash
pip install grepify[turbopuffer]
```
...
The `turbopuffer` connector provides target state APIs for writing rows to namespaces. Grepify tracks what rows should exist and automatically handles upserts and deletions. Turbopuffer creates namespaces implicitly on the first write, so there is no separate "create namespace" step — but the connector still tracks namespace-level configuration (vector schema and distance metric) and clears the namespace if it must be rebuilt.
...
:::note
The key name is load-bearing across runs — it's the stable identity Grepify uses to track managed namespaces. See [ContextKey as stable identity](../programming_guide/context#contextkey-as-stable-identity) before renaming.
:::
...
- `managed_by` — Whether Grepify manages the namespace lifecycle (`"system"`) or assumes it exists (`"user"`).
