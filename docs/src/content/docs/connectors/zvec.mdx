...
```python
from grepify.connectors import zvec
```
...
:::note
The key name is load-bearing across runs — it's the stable identity Grepify uses to track managed documents. See [ContextKey as stable identity](../programming_guide/context#contextkey-as-stable-identity) before renaming.
:::
...
- `managed_by` — Whether Grepify manages the collection lifecycle (`"system"`, creating and destroying it) or assumes it already exists (`"user"`, documents only).
...
```python
from grepify.connectors.zvec import ZvecVectorDef
