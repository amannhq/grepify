// Locates the agent skill (skills/grepify/) in the repo, a sibling of this
...
const SKILL_DIR = [
  resolve(process.cwd(), '../skills/grepify'),
  resolve(process.cwd(), 'skills/grepify'),
].find(existsSync);

if (!SKILL_DIR) {
  throw new Error('skills/grepify not found relative to the docs build cwd');
}
