...
function buildGuard(slug: string, lead: string, extra = ''): string {
  return (
    ...
    `\`@grepify.flow_def\`/\`FlowBuilder\` → \`coco.App\` + a \`@coco.fn\` main function; ` +
    `\`add_collector()\`/\`collect()\`/\`export()\` → declare target states (\`declare_row\`, \`declare_file\`); ` +
    `\`grepify.sources/functions/targets.*\` → connector APIs (\`localfs.walk_dir\`, \`coco.ops.*\`, \`postgres.declare_table_target\`). ` +
...
export function buildDocMarkdown(doc: CollectionEntry<'docs'>): string {
  ...
  const guard = buildGuard(
    slug,
    `**Grepify v1.** This page documents Grepify **v1** — a ground-up redesign ` +
