...
The same pipeline is written twice — once with the Grepify Rust SDK, once
...
Every step uses a Grepify primitive:
...
A single cold run doesn't tell you much. Real Grepify users care about what
...
│  ├─ benchmark.py        ← Grepify Python SDK pipeline
│  ├─ common.py           ← dataset generator, mutations, Python reference impl
│  └─ pyproject.toml      ← uv project pinning the local grepify editable install
├─ rust/                  ← Grepify Rust SDK pipeline (Cargo crate)
│  ├─ Cargo.toml
│  └─ src/main.rs
