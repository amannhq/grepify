...
impl Scenario {
    fn parse(value: &str) -> grepify::Result<Self> {
        match value {
...
async fn extract_sections(
    ...
    file: &FileEntry,
) -> grepify::Result<Vec<SectionInput>> {
...
) -> grepify::Result<SectionAnalysis> {
...
) -> grepify::Result<CollectionSummary> {
...
async fn main() -> grepify::Result<()> {
    ...
    if let Some(parent) = args.metrics.parent() {
        fs::create_dir_all(parent).map_err(grepify::Error::Io)?;
    }
    fs::write(&args.metrics, metric_bytes).map_err(grepify::Error::Io)?;
    Ok(())
}
...
fn parse_args() -> grepify::Result<Args> {
...
fn required_arg<'a>(map: &'a HashMap<String, String>, flag: &str) -> grepify::Result<&'a str> {
    map.get(flag)
        .map(String::as_str)
        .ok_or_else(|| engine_err(format!("missing required argument `{flag}`")))
}

fn discover_collections(root: &Path) -> grepify::Result<Vec<CollectionDir>> {
    let mut collections = Vec::new();
    for entry in fs::read_dir(root).map_err(grepify::Error::Io)? {
        let entry = entry.map_err(grepify::Error::Io)?;
        let path = entry.path();
        if path.is_dir() {
...
fn sync_output_tree(
...
) -> grepify::Result<OutputSyncStats> {
    let settings = profile.settings();
    ...
    fs::create_dir_all(output_root).map_err(grepify::Error::Io)?;
    ...
    for (rel_path, content) in &desired {
        ...
        if current.as_deref() != Some(content.as_slice()) {
            fs::write(&path, content).map_err(grepify::Error::Io)?;
    ...
    for rel_path in existing {
        ...
        fs::remove_file(output_root.join(&rel_path)).map_err(grepify::Error::Io)?;
        output_files_rebuilt += 1;
        if rel_path.starts_with(&format!("{collection_dir}/")) {
            collection_rebuilt += 1;
        }
...
fn collect_existing_json(
    ...
    for entry in fs::read_dir(current).map_err(grepify::Error::Io)? {
        let entry = entry.map_err(grepify::Error::Io)?;
        let path = entry.path();
        if path.is_dir() {
...
fn canonical_json_bytes<T: Serialize>(value: &T) -> grepify::Result<Vec<u8>> {
...
fn write_canonical_json(value: &Value, out: &mut Vec<u8>) -> grepify::Result<()> {
    match value {
...
fn tree_digest(root: &Path) -> grepify::Result<String> {
    ...
    for rel_path in files {
        ...
        hasher.update(b"\0");
        hasher.update(&fs::read(root.join(&rel_path)).map_err(grepify::Error::Io)?);
        hasher.update(b"\0");
    }
    Ok(hasher.hexdigest())
}

fn collect_all_files(root: &Path, current: &Path, out: &mut Vec<PathBuf>) -> grepify::Result<()> {
    for entry in fs::read_dir(current).map_err(grepify::Error::Io)? {
        let entry = entry.map_err(grepify::Error::Io)?;
        let path = entry.path();
        if path.is_dir() {
...
fn engine_err(message: impl Into<String>) -> grepify::Error {
    grepify::Error::engine(message.into())
}
