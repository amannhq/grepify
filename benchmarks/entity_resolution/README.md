...
Synthetic benchmark for `grepify.ops.entity_resolution.resolve_entities`.
...
The benchmark measures logical `embed(...)` calls. Grepify embedders such as
...
The benchmark sets `GREPIFY_DB` to `.work/grepify` by default so memoized
Grepify functions, such as the LiteLLM embedder and LLM resolver, have a local
state path.
