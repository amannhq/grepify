...
use grepify_utils::error::{SharedError, SharedResult};
...
fn make_handle(rx: oneshot::Receiver<HandleOutcome>) -> ComponentExecutionHandle {
    ComponentExecutionHandle::new(async move {
        match rx.await {
            Ok(HandleOutcome::Executed(Ok(()))) => grepify_utils::error::shared_ok(()),
            Ok(HandleOutcome::Executed(Err(e))) => Err(e),
            Ok(HandleOutcome::Superseded) => grepify_utils::error::shared_ok(()),
