...
impl AppStore {
    ...
    pub async fn read_txn<'a>(&'a self) -> Result<ReadTxn<'a>> {
        ...
        let txn = match grepify_utils::retryable::run(&try_open, &READ_TXN_RETRY_PHASE1).await {
            Ok(txn) => txn,
            ...
            Err(_) => {
                ...
                grepify_utils::retryable::run(&try_open, &READ_TXN_RETRY_PHASE2)
                    .await
...
static READ_TXN_RETRY_PHASE1: grepify_utils::retryable::RetryOptions =
    grepify_utils::retryable::RetryOptions {
        retry_timeout: Some(std::time::Duration::from_secs(3)),
        initial_backoff: std::time::Duration::from_millis(10),
        max_backoff: std::time::Duration::from_secs(1),
    };

static READ_TXN_RETRY_PHASE2: grepify_utils::retryable::RetryOptions =
    grepify_utils::retryable::RetryOptions {
        retry_timeout: None,
        initial_backoff: std::time::Duration::from_millis(10),
        max_backoff: std::time::Duration::from_secs(1),
        ...
        self.run_in_batcher(move |wtxn| {
            ...
            Box::pin(async move {
                ...
                let mut info: crate::state::db_schema::StablePathEntryTrackingInfo<'_> =
                    grepify_utils::deser::from_msgpack_slice(bytes)?;
...
mod tests {
    ...
    async fn read_pending_process_token(store: &AppStore, path: &StablePath) -> Option<u128> {
        ...
        let info: StablePathEntryTrackingInfo<'_> =
            grepify_utils::deser::from_msgpack_slice(&bytes).unwrap();
