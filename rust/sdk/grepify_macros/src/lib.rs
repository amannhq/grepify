//! Proc macros for grepify: `#[function]`.
    ...
    match memo_key_override(overrides, ident) {
        ...
        Some(override_) => {
            ...
            Some(quote! {
                ...
                ::grepify::memo::write_key_fingerprint_part(&mut #fingerprinter, &#temp)?;
            })
        }
        None if param.is_str_ref => Some(quote! {
            ::grepify::memo::write_key_fingerprint_part(&mut #fingerprinter, #default_arg)?;
        }),
        None => Some(quote! {
            ::grepify::memo::write_key_fingerprint_part_for_arg(&mut #fingerprinter, #default_arg)?;
        }),
    }
}
...
fn gen_state_collect_for_param(
    ...
    quote! {
        ...
        if let Some(__coco_state) =
            ::grepify::memo::collect_memo_arg_state(#default_arg, __coco_prev_state).await?
        {
...
/// `#[grepify::function]` — unified macro for grepify pipeline functions.
///
/// ## Usage
///
...
/// ```ignore
/// #[grepify::function(memo, version = 2)]
/// async fn my_fn(ctx: &Ctx, arg: &String) -> Result<String> { ... }
/// ```
...
pub fn function(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let logic_registration = if track_logic {
        quote! {
            #[::grepify::linkme::distributed_slice(::grepify::COCO_FN_LOGIC)]
            #[linkme(crate = ::grepify::linkme)]
            #[doc(hidden)]
            static #logic_slice_name: ::grepify::FnLogicEntry = ::grepify::FnLogicEntry {
    ...
    if args.memo {
        ...
        let expanded = quote! {
            ...
            #vis #sig {
                let __coco_key = {
                    let mut __coco_key = ::grepify::memo::new_key_fingerprinter();
                    ::grepify::memo::write_key_fingerprint_part(&mut __coco_key, &"grepify_fn")?;
                    ::grepify::memo::write_key_fingerprint_part(&mut __coco_key, &::core::module_path!())?;
                    ::grepify::memo::write_key_fingerprint_part(&mut __coco_key, &::core::stringify!(#fn_name))?;
                    ::grepify::memo::write_key_fingerprint_part(&mut __coco_key, &#hash_const_name)?;
                    #(#key_writes)*
                    ::grepify::memo::finish_key_fingerprinter(__coco_key)
                };

                ::grepify::memo::cached_by_fingerprint_with_state(#ctx_ident, __coco_key, #propagate_children_fn_logic, {
                    #(#clone_stmts)*
...
pub fn use_mount(input: TokenStream) -> TokenStream {
    ...
    quote! {{
        ...
        #(#arg_binds)*
        let __coco_memo_fp = ::grepify::mount::component_memo_fp(
            ::core::module_path!(),
...
pub fn mount_each(input: TokenStream) -> TokenStream {
    ...
    quote! {{
        ...
        let __coco_memo_of = {
            ...
            move |__coco_value: &_| ::grepify::mount::component_memo_fp(
                ::core::module_path!(),
                ::core::stringify!(#fn_ident),
                #hash_path,
                &( #(#fp_elems,)* ),
        ...
        let __coco_body = {
            #( let #extra_idents = ::core::clone::Clone::clone(&#extra_idents); )*
            move |__coco_child: ::grepify::Ctx, __coco_value| {
                #( let #extra_idents = ::core::clone::Clone::clone(&#extra_idents); )*
...
fn logical_type_tokens(ty: &Type, attr: &SchemaFieldAttr) -> TokenStream2 {
    if let Some(t) = &attr.custom_type {
        return quote! { ::grepify::LogicalType::Custom(::std::string::String::from(#t)) };
    }
    if let Some(dim) = attr.vector_dim {
        let half = attr.vector_half;
        return quote! { ::grepify::LogicalType::Vector { dim: #dim, half: #half } };
    }
    if attr.force_json {
        return quote! { ::grepify::LogicalType::Json };
    }
    ...
    quote! { ::grepify::LogicalType::#variant }
}
...
/// See [`grepify::row_schema`] for the `#[coco(...)]` field attributes.
...
pub fn derive_schema_fields(input: TokenStream) -> TokenStream {
    ...
    quote! {
        impl #impl_generics ::grepify::SchemaFields for #name #ty_generics #where_clause {
            fn schema_fields() -> ::std::vec::Vec<::grepify::SchemaField> {
                ::std::vec![ #(#entries),* ]
            }
        }
    }
...
mod tests {
    ...
    fn parse_fn_params_rejects_multiple_ctx_params() {
        let func: ItemFn =
            parse_str("async fn bad(ctx: &Ctx, other: &grepify::Ctx) -> Result<i32, ()> { Ok(0) }")
                .unwrap();
        assert!(parse_fn_params(&func).is_err());
    }
    ...
    fn parse_fn_params_accepts_qualified_ctx_type() {
        let func: ItemFn = parse_str(
            "async fn good(ctx: &grepify::Ctx, value: &str) -> Result<i32, ()> { Ok(0) }",
        )
        .unwrap();
