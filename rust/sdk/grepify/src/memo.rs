...
use grepify_core::engine::context::{FnCallContext, MemoStatesPayload};
use grepify_core::engine::function::reserve_memoization;
use grepify_utils::fingerprint::{Fingerprint, Fingerprinter};
...
fn fallback_key_bytes<T: ?Sized>(error: Error) -> Vec<u8> {
    ...
    match rmp_serde::to_vec_named(&(
        "__grepify_key_bytes_unsupported__",
        std::any::type_name::<T>(),
        seq,
        error.to_string(),
    )) {
        Ok(bytes) => bytes,
        Err(_) => {
            let mut bytes = b"__grepify_key_bytes_unsupported__".to_vec();
...
mod tests {
    ...
    fn key_bytes_uses_fallback_on_serialization_error() {
        ...
        assert_eq!(marker, "__grepify_key_bytes_unsupported__");
