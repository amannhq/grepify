...
use grepify_core::engine::component::OnError;
use grepify_core::engine::live_component::LiveComponentController;
use grepify_core::state::stable_path::{StableKey, StablePath};
...
type CoreError = grepify_utils::error::Error;
...
pub(crate) fn build_chained_on_error(
    ...
    Some(Arc::new(move |err: CoreError| {
        ...
        Box::pin(async move {
            ...
            as Pin<Box<dyn Future<Output = grepify_utils::error::Result<()>> + Send + 'static>>
    }))
}
