...
//! batch call (via the core batcher, `grepify_utils::batching`).
//!
...
//!
//! ```ignore
//! #[grepify::function]                                   // ctx-free batch impl; emits a logic hash
...
type BatchFuture<Out> =
    Pin<Box<dyn Future<Output = grepify_utils::error::Result<Vec<Out>>> + Send>>;
...
impl<In: Send + 'static, Out: Send + 'static> Runner for FnRunner<In, Out> {
    ...
    async fn run(
        &self,
        inputs: Vec<In>,
    ) -> grepify_utils::error::Result<impl ExactSizeIterator<Item = Out>> {
    ...
    Out: Serialize + DeserializeOwned + Send + 'static,
{
    ...
    /// `__COCO_FN_HASH_*` constant emitted by `#[grepify::function]`. It is
        ...
        Fut: Future<Output = Result<Vec<Out>>> + Send + 'static,
    {
        let wrapped: BatchFn<In, Out> = Box::new(move |inputs| {
            ...
            Box::pin(async move {
                fut.await
                    .map_err(|e: Error| grepify_utils::error::Error::internal_msg(e.to_string()))
            })
        });
