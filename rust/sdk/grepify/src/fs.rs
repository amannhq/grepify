...
/// # Examples
/// ```ignore
/// let files = grepify::fs::walk("./src", &["**/*.rs", "**/*.toml"])?;
/// ```
...
/// the previous run via Grepify's target-state engine:
...
pub fn declare_dir_target(ctx: &Ctx, target: DirTargetState) -> Result<DirTarget> {
    ...
    let provider = register_root_target_states_provider(
        ctx,
        format!("grepify/localfs/dir/{}", dir.to_string_lossy()),
...
impl DirTarget {
    ...
    ///
    /// ```no_run
    /// # use grepify::{ctx::Ctx, fs::DirTarget};
