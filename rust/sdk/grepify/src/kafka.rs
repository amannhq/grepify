...
//!
//! Topics are user-managed: Grepify never creates or drops them during target
...
impl KafkaProducer {
    ...
    ///
    /// Idempotent and explicit — this is *not* part of reconciliation (Grepify
...
pub fn mount_kafka_topic_target(
    ...
    let messages = register_root_target_states_provider(
        ctx,
        format!("grepify/kafka/topic/{}/{}", producer.state_id(), topic),
        MessageHandler::new(
