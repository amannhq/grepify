...
use grepify_core::engine::app::{App as CoreApp, AppOpHandle, AppUpdateOptions};
use grepify_core::engine::environment::{Environment as CoreEnvironment, EnvironmentSettings};
use grepify_core::engine::progress_display::{ProgressDisplayOptions, show_progress};
use grepify_core::engine::stats::{ProcessingStats, TERMINATED_VERSION};
use grepify_core::engine::target_state::TargetStateProviderRegistry;
...
/// A Grepify environment: the LMDB store plus the resources shared with every
...
///
/// ```no_run
/// # use grepify::Environment;
/// # async fn run() -> grepify::error::Result<()> {
...
impl App {
    ...
    /// ```no_run
    /// # use grepify::app::App;
    /// # async fn doc() -> grepify::error::Result<()> {
    /// let app = App::open("my_app", "./data").await?;
    ...
    // -----------------------------------------------------------------------
    // Inspect API (read-only). These delegate to `grepify_core::inspect` and
    // return the engine's public inspect types, so alternate hosts (e.g. the
    // napi Node binding) can expose LMDB inspection without needing access to
    // the sealed `RustProfile`.
    // -----------------------------------------------------------------------

    /// List every stable path recorded for this app.
    pub async fn inspect_list_stable_paths(&self) -> Result<Vec<StablePath>> {
        db_inspect::list_stable_paths(&self.inner.core_app)
            .await
            .map_err(|e| Error::engine(format!("{e}")))
    }

    /// Collect all stable paths together with their node-type metadata.
    pub async fn inspect_stable_path_infos(&self) -> Result<Vec<db_inspect::StablePathInfo>> {
        use futures::StreamExt;
        let stream = db_inspect::iter_stable_paths(&self.inner.core_app).await;
        stream
            .collect::<Vec<_>>()
            .await
            .into_iter()
    ...
    /// Detailed info (version, processor, target-state summary) for one path.
    pub async fn inspect_stable_path_detail(
        &self,
        path: &StablePath,
    ) -> Result<Option<db_inspect::StablePathDetail>> {
        db_inspect::get_stable_path_detail(&self.inner.core_app, path)
            .await
            .map_err(|e| Error::engine(format!("{e}")))
    }

    /// Detailed info for a path and (optionally) its children/parents.
    pub async fn inspect_query_stable_path_details(
        &self,
        path: &StablePath,
        include_children: bool,
        recursive: bool,
        include_parents: bool,
    ) -> Result<Vec<db_inspect::StablePathDetail>> {
...
impl Environment {
    // -----------------------------------------------------------------------
    // Inspect API (read-only), environment/by-name variants. See `App`'s
    // inspect methods for rationale.
    // -----------------------------------------------------------------------

    /// List the names of all apps that have persisted state in this environment.
    pub async fn inspect_list_app_names(&self) -> Result<Vec<String>> {
        db_inspect::list_app_names(&self.inner.core_env)
            .await
            .map_err(|e| Error::engine(format!("{e}")))
    }

    /// Collect the stable paths (with node-type metadata) of an app by name,
    /// opening its store read-only. Empty if the app does not exist.
    pub async fn inspect_stable_path_infos_by_name(
        &self,
        app_name: &str,
    ) -> Result<Vec<db_inspect::StablePathInfo>> {
        use futures::StreamExt;
        let stream = db_inspect::iter_stable_paths_by_name(&self.inner.core_env, app_name)
            .await
            .map_err(|e| Error::engine(format!("{e}")))?;
        stream
            .collect::<Vec<_>>()
    ...
    pub async fn inspect_stable_path_detail_by_name(
        &self,
        app_name: &str,
        path: &StablePath,
    ) -> Result<Option<db_inspect::StablePathDetail>> {
        db_inspect::get_stable_path_detail_by_name(&self.inner.core_env, app_name, path)
            .await
            .map_err(|e| Error::engine(format!("{e}")))
    }

    /// Detailed info for a path (and optionally children/parents) of an app by name.
    pub async fn inspect_query_stable_path_details_by_name(
        &self,
        app_name: &str,
        path: &StablePath,
        include_children: bool,
