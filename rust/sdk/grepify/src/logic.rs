...
//!
//! Each `#[grepify::function]` adds one [`FnLogicEntry`] to the [`COCO_FN_LOGIC`]
...
use grepify_core::engine::logic_registry;
...
/// `#[grepify::function]` records at call time (see `Ctx::__coco_tracked_fn`):
/// `Fingerprint::from(&("grepify_fn", module, name, code_hash))`.
...
fn entry_fingerprint(e: &FnLogicEntry) -> Option<Fingerprint> {
    Fingerprint::from(&("grepify_fn", e.module, e.name, e.code_hash)).ok()
}
