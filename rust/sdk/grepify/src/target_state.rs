...
use grepify_core::engine::target_state::{ChildInvalidation, TargetActionSinkKeeper};
pub use grepify_core::state::stable_path::StableKey;
...
pub struct TargetStateProvider<V> {
    pub(crate) inner: grepify_core::engine::target_state::TargetStateProvider<RustProfile>,
...
impl<V> TargetStateProvider<V> {
    pub(crate) fn new(
        inner: grepify_core::engine::target_state::TargetStateProvider<RustProfile>,
    ) -> Self {
        Self {
            inner,
            _value: PhantomData,
        ...
        let inner = TargetActionSinkKeeper::new(BoxedSink::new(move |actions| {
            ...
            Box::pin(async move {
                ...
                let mapped = defs
                    .into_iter()
                    .map(|d| {
                        d.map(|d| grepify_core::engine::target_state::ChildTargetDef {
                            handler: d.handler,
                        })
                    })
                    .collect();
    ...
    ChildV: Serialize + DeserializeOwned + Send + 'static,
{
    ...
    type CoreProvider = grepify_core::engine::target_state::TargetStateProvider<RustProfile>;
    ...
    H: TargetHandler<V>,
{
    ...
    BoxedHandler::new(move |key, desired, prev, prev_may_be_missing| {
        ...
        Ok(Some(
            grepify_core::engine::target_state::TargetReconcileOutput {
                action,
...
fn internal(err: impl std::fmt::Display) -> grepify_utils::error::Error {
    grepify_utils::error::Error::internal_msg(err.to_string())
}
