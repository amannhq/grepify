...
//! deleted. `managed_by` controls whether Grepify owns table DDL.
//!
...
/// A LanceDB column type (the subset Grepify maps natively).
...
pub fn table_target_with_options(
    ...
    let provider = register_root_target_states_provider(
        ctx,
        format!("grepify/lancedb/table/{}/{}", db.name(), table_name),
        TableHandler {
...
/// path uses Grepify's child-provider invalidation directly. This sync helper
...
struct VectorIndexAction {
    ...
    /// dropped by Grepify — it doesn't own that DDL.
    managed_by: ManagedBy,
}
...
async fn apply_vector_index(db: &LanceDatabase, action: VectorIndexAction) -> Result<()> {
    ...
    // User-managed indexes: Grepify owns neither their creation nor their
...
async fn apply_fts_index(db: &LanceDatabase, action: FtsIndexAction) -> Result<()> {
    ...
    // by Grepify; guard on the action's `managed_by` so the drop path respects it.
    if action.managed_by.is_user() {
        return Ok(());
    }
    if !table_exists(db, &action.table_name).await? {
