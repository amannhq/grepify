...
//!
//! Mirrors Python's `grepify.ops.code`. Parse source once into a [`CodeAst`],
...
//! use grepify::ops::code::{CodeAst, CodePattern, match_code};
//!
...
use grepify_code_ast::tree_sitter::Tree;
use grepify_code_ast::{LineIndex, OutputPosition, ParseOutcome};
use grepify_code_match::lang;
use grepify_code_match::{Pattern, Prefilter, index_terms_in_tree};
use grepify_ops_text::split::{RecursiveChunkConfig, RecursiveChunker, RecursiveSplitConfig};
...
fn build_sdk_matches(
    ...
    raw: Vec<grepify_code_match::Match<'_>>,
) -> Vec<CodeMatch> {
    raw.into_iter()
        .map(|m| {
