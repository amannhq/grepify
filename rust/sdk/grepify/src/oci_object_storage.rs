...
//! Python's `grepify.connectors.oci_object_storage`:
//!
...
mod tests {
    ...
    async fn oci_file_is_recognized_by_memo_arg_helpers() {
        ...
        let mut file_key = grepify_utils::fingerprint::Fingerprinter::default();
        crate::memo::write_key_fingerprint_part_for_arg(&mut file_key, &file).unwrap();

        let mut expected_key = grepify_utils::fingerprint::Fingerprinter::default();
