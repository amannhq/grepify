...
//!
//! Fully hermetic: it runs a real Grepify pipeline into a LanceDB database in a
...
//! delete-orphan).
//!
//!   cargo test -p grepify --features lancedb --test lancedb_target
#![cfg(feature = "lancedb")]

use grepify::lancedb::{self, ColumnDef, ColumnType, LanceDatabase, TableSchema};
use grepify::{ContextKey, Environment, ManagedTargetOptions, Result};
use serde::Serialize;
...
async fn lancedb_target_creates_upserts_searches_and_reconciles() -> Result<()> {
    ...
    let coco_db_path = tempdir.path().join(".grepify_db");
...
/// A user-managed FTS index must NOT be dropped by Grepify when it stops being
/// declared — Grepify doesn't own user-managed DDL. Regression test for the
...
async fn lancedb_user_managed_fts_index_not_dropped_on_undeclare() -> Result<()> {
    ...
    // Stage 2: a *user-managed* Grepify deployment (separate tracking db) adopts
    // the existing table + index, then stops declaring the index. The index must
    // survive — Grepify doesn't own it.
