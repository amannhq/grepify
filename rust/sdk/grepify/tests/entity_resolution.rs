...
impl EntityEmbedder for ScriptedEmbedder {
    async fn embed_entity(&self, entity: &str) -> grepify::Result<Vec<f32>> {
        self.vectors
            .get(entity)
            .cloned()
            .ok_or_else(|| grepify::Error::engine(format!("missing vector for {entity}")))
    }
}
