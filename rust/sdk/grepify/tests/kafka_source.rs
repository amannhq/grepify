...
//!     cargo test -p grepify --features kafka --test kafka_source
//!
...
use grepify::{App, Result, UpdateOptions, kafka};
...
fn unique_topic(label: &str) -> String {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    format!("grepify_kafka_src_{label}_{nonce}")
}
...
async fn kafka_source_reads_all_partitions() -> Result<()> {
    ...
    let raw = ClientBuilder::new(brokers.clone())
        .build()
        .await
        .map_err(|e| grepify::Error::engine(format!("client: {e}")))?;
    for (pid, key, val) in [(0, "k0", "v0"), (1, "k1", "v1"), (2, "k2", "v2")] {
        let pc = raw
            .partition_client(topic.clone(), pid, UnknownTopicHandling::Retry)
            .await
            .map_err(|e| grepify::Error::engine(format!("partition_client: {e}")))?;
        let record = Record {
        ...
        pc.produce(vec![record], Compression::NoCompression)
            .await
            .map_err(|e| grepify::Error::engine(format!("produce p{pid}: {e}")))?;
    }
