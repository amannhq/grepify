...
use grepify::{ContextKey, Ctx, Environment, Result, postgres};
...
async fn relation_exists(
    ...
    .fetch_one(db.pool())
    .await
    .map_err(|e| grepify::Error::engine(format!("postgres relation lookup: {e}")))
}
...
async fn postgres_table_target_reconciles_rows_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_test_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    sqlx::query(&format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE"))
        .execute(db.pool())
        .await
        .map_err(|e| grepify::Error::engine(format!("postgres cleanup: {e}")))?;

    let tempdir = tempfile::tempdir().unwrap();
    let app = Environment::builder()
        .db_path(tempdir.path().join(".grepify_db"))
    ...
    .fetch_all(db.pool())
    .await
    .map_err(|e| grepify::Error::engine(format!("postgres query: {e}")))?;
    let actual = rows
        .into_iter()
        .map(|row| {
    ...
    sqlx::query(&format!("DROP TABLE \"{schema}\".\"rows\""))
        .execute(db.pool())
        .await
        .map_err(|e| grepify::Error::engine(format!("postgres external table drop: {e}")))?;
    app.run({
        let schema = schema.clone();
    ...
    let row_count: i64 = sqlx::query_scalar(&format!("SELECT count(*) FROM \"{schema}\".\"rows\""))
        .fetch_one(db.pool())
        .await
        .map_err(|e| grepify::Error::engine(format!("postgres count: {e}")))?;
...
async fn postgres_sql_command_attachment_runs_setup_and_teardown_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_sqlcmd_test_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    sqlx::query(&format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE"))
        .execute(db.pool())
        .await
...
async fn postgres_read_table_items_keys_rows_when_available() -> Result<()> {
    ...
    .execute(db.pool())
    .await
    .map_err(|e| grepify::Error::engine(format!("postgres create source table: {e}")))?;
    sqlx::query(&format!(
        "INSERT INTO \"{table}\" (id, name) VALUES (1, 'a'), (2, 'b')"
    ))
    .execute(db.pool())
    .await
    .map_err(|e| grepify::Error::engine(format!("postgres insert: {e}")))?;
    ...
    let mut items: Vec<(grepify::StableKey, SrcRow)> =
        postgres::read_table_items(&db, &table, |row: &SrcRow| row.id).await?;
    items.sort_by_key(|(_, row)| row.id);
    assert_eq!(items.len(), 2);
    assert_eq!(items[0].0, grepify::StableKey::Int(1));
    assert_eq!(items[0].1.name, "a");
    assert_eq!(items[1].0, grepify::StableKey::Int(2));
    assert_eq!(items[1].1.name, "b");

    sqlx::query(&format!("DROP TABLE \"{table}\""))
        .execute(db.pool())
        .await
        .map_err(|e| grepify::Error::engine(format!("postgres drop source table: {e}")))?;
    Ok(())
}
...
async fn postgres_vector_index_target_reconciles_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_vector_test_{nonce}");
    let db = postgres::Database::connect(&url).await?;
...
async fn postgres_strips_nul_from_text_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_nul_test_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    sqlx::query(&format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE"))
        .execute(db.pool())
        .await
...
async fn column_exists(
    ...
    .fetch_one(db.pool())
    .await
    .map_err(|e| grepify::Error::engine(format!("postgres column lookup: {e}")))?;
    Ok(count > 0)
}
...
async fn postgres_adds_and_drops_columns_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_evolve_test_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    sqlx::query(&format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE"))
        .execute(db.pool())
        .await
...
async fn postgres_column_drop_retries_after_failed_attempt_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_dropretry_test_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    sqlx::query(&format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE"))
        .execute(db.pool())
        .await
    ...
    .execute(db.pool())
    .await
    .map_err(|e| grepify::Error::engine(format!("postgres create view: {e}")))?;
    ...
    sqlx::query(&format!("DROP VIEW \"{schema}\".\"dep\""))
        .execute(db.pool())
        .await
        .map_err(|e| grepify::Error::engine(format!("postgres drop view: {e}")))?;
    app.run({
        let schema = schema.clone();
        move |ctx| {
            declare_cols(
...
async fn postgres_bytea_round_trips_byte_arrays_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_bytea_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    sqlx::query(&format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE"))
        .execute(db.pool())
        .await
...
async fn postgres_schema_evolution_retype_and_pk_change_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let schema = format!("grepify_rust_evo_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    sqlx::query(&format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE"))
        .execute(db.pool())
        .await
