...
//!   NEO4J_URI=bolt://localhost:7687 NEO4J_PASSWORD=grepify \
//!   FALKORDB_URI=falkor://localhost:6379 \
//!     cargo test -p grepify --features neo4j,falkordb --test graph_vector_index
//!
...
async fn neo4j_vector_index_create_then_drop_when_available() {
    use grepify::neo4j::{self, ColumnDef, TableSchema, VectorMetric};
    use grepify::{App, ContextKey, Environment, Result};
    ...
    let password = std::env::var("NEO4J_PASSWORD").unwrap_or_else(|_| "grepify".to_string());
...
async fn falkordb_vector_index_create_then_drop_when_available() {
    use grepify::falkordb::{self, ColumnDef, TableSchema, VectorMetric};
    use grepify::{App, ContextKey, Environment, Result};
    use std::sync::LazyLock;
    use std::time::{SystemTime, UNIX_EPOCH};
