...
//!     cargo test -p grepify --features amazon_s3 --test amazon_s3_source
#![cfg(feature = "amazon_s3")]

use std::sync::Arc;
use std::time::{SystemTime, UNIX_EPOCH};

use grepify::amazon_s3::aws_sdk_s3::primitives::ByteStream;
use grepify::amazon_s3::{self, ListOptions, S3Client, S3File};
use grepify::file::PatternFilePathMatcher;
use grepify::{App, Environment, FileLike, Result};
...
async fn s3_source_mount_each_pipeline_when_available() -> Result<()> {
    ...
    static S3: std::sync::LazyLock<grepify::ContextKey<S3Client>> =
        std::sync::LazyLock::new(|| {
            grepify::ContextKey::new_with_state("s3_test_client", |c: &S3Client| {
                c.state_id().to_string()
            })
        });

    #[grepify::function(memo)]
    async fn process(ctx: &grepify::Ctx, file: &S3File) -> Result<usize> {
        ...
        .update(move |ctx| {
            ...
            async move {
                ...
                let sizes = ctx
                    ...
                    .await?;
                Ok::<usize, grepify::Error>(sizes.into_iter().sum())
            }
        })
        .await?;
