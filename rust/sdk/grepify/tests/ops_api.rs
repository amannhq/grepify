...
//!
//! Run with: `cargo test -p grepify --features embed_api --test ops_api`.
...
use grepify::ops::api::{ApiEmbedder, ApiTranscriber, LlmPairResolver};
use grepify::prelude::{FileContentCache, FileLike, FileMetadata, FilePath};
use grepify::{CanonicalSide, PairResolver};
use serde_json::json;
...
async fn api_embedder_vector_schema() {
    ...
    use grepify::VectorSchemaProvider;
    let schema = embedder.vector_schema().await.unwrap();
    assert_eq!(schema.size, 4);
    assert_eq!(schema.element_type, grepify::VectorElementType::Float32);
}
...
impl FileLike for InMemoryFile {
    ...
    async fn fetch_metadata(&self) -> grepify::Result<FileMetadata> {
        Ok(FileMetadata {
    ...
    async fn read_impl(&self, _size: Option<usize>) -> grepify::Result<Vec<u8>> {
        Ok(self.data.clone())
    }
}
