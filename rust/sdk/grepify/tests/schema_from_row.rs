...
fn doris_from_row_matches_explicit_schema() {
    use grepify::SchemaFields;
    use grepify::doris::{ColumnDef, TableSchema};
...
fn sqlite_from_row_matches_explicit_schema() {
    use grepify::SchemaFields;
    use grepify::sqlite::{ColumnDef, TableSchema};
...
fn postgres_from_row_matches_explicit_schema() {
    use grepify::SchemaFields;
    use grepify::postgres::{ColumnDef, TableSchema};
...
#[cfg(feature = "sqlite")]
#[tokio::test]
async fn sqlite_from_row_round_trips_a_row() -> grepify::Result<()> {
    use grepify::sqlite::{self, Database, TableSchema};
    use grepify::{ContextKey, Environment, SchemaFields};
    use sqlx::Row as _;
...
async fn doris_from_row_round_trips_a_row() -> grepify::Result<()> {
    use grepify::doris::{self, DorisConfig, DorisConnection, TableSchema};
    use grepify::{ContextKey, Environment, SchemaFields};
    use sqlx::Row as _;
    ...
    let database = std::env::var("DORIS_DATABASE").unwrap_or_else(|_| "grepify_test".to_string());
