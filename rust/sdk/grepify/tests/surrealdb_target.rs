...
use grepify::prelude::*;
use grepify::surrealdb::{self, ColumnDef, Graph, TableSchema, VectorIndexOptions};
...
async fn try_graph(db_name: &str) -> Option<Graph> {
    ...
    let ns = std::env::var("SURREALDB_NS").unwrap_or_else(|_| "grepify".to_string());
...
async fn run_conversation_graph(app: &App, include_all_entities: bool) {
    app.run(move |ctx| async move {
        ...
        statement.declare_record(
            &ctx,
            200_i64,
            &serde_json::json!({ "statement": "Grepify writes Rust targets" }),
        )?;
        ...
        if include_all_entities {
            ...
            org.declare_record(&ctx, "Grepify", &serde_json::json!({ "name": "Grepify" }))?;
            statement_mentions.declare_relation_between(
                &ctx,
                "statement",
                200_i64,
