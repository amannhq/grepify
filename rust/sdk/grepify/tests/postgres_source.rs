...
//!   POSTGRES_URL=postgres://grepify:grepify@localhost:5544/grepify \
//!     cargo test -p grepify --features postgres --test postgres_source
...
async fn postgres_source_reads_processes_and_reconciles_when_available() -> Result<()> {
    ...
    let out_schema_name = format!("grepify_src_test_{nonce}");
    let db = postgres::Database::connect(&url).await?;
    let pe = |e: sqlx::Error| grepify::Error::engine(format!("pg: {e}"));
    ...
    {
        let src_schema = format!("grepify_src_schema_{nonce}");
    ...
    let db_path = tempdir.path().join(".grepify_db");
