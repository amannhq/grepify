...
//! compile this file with `unresolved import grepify::ops::code`.
#![cfg(feature = "code_match")]

use grepify::ops::code::{CodeAst, CodePattern, FileMatch, index_terms, match_code};
...
fn language_aliases_accepted() {
    // Languages/aliases resolved through the `grepify_code_ast` registry,
...
fn parse_uses_treesitter_table_like_python() {
    // CodeAst parsing goes through the `grepify_code_ast` registry, exactly
