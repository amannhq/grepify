...
//!     cargo test -p grepify --features kafka --test kafka_target
//!
...
async fn kafka_target_produces_skips_updates_and_tombstones_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let topic = format!("grepify_kafka_test_{nonce}");
    ...
    let consumer = ClientBuilder::new(broker_refs.iter().map(|s| s.to_string()).collect())
        .build()
        .await
        .map_err(|e| grepify::Error::engine(format!("consumer connect: {e}")))?;
...
async fn kafka_target_custom_delete_value_when_available() -> Result<()> {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let topic = format!("grepify_kafka_delval_test_{nonce}");
