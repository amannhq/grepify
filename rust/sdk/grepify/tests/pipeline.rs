...
fn update_blocking_returns_main_result() {
    ...
    let result = app
        .update_blocking(|_ctx| async move { Ok::<_, grepify::Error>("done".to_string()) })
        .unwrap();
...
async fn update_async_returns_main_result() {
    ...
    let result = app
        .update(|_ctx| async move { Ok::<_, grepify::Error>(42i32) })
        .await
        .unwrap();
    assert_eq!(result, 42);
}
...
async fn start_update_handle_returns_main_result() {
    ...
    let mut handle = app
        .start_update(|_ctx| async move { Ok::<_, grepify::Error>(7i32) })
        .unwrap();
...
async fn start_update_handle_result_propagates_errors() {
    ...
    let handle = app
        .start_update(|_ctx| async move { Err::<(), _>(grepify::Error::engine("handle boom")) })
        .unwrap();
    ...
    async fn drive(
        ...
        app.update(move |ctx| async move {
            ...
            let fut_a = ctx.memo(&"memoA", move |ctx| async move {
                ...
                Ok::<_, grepify::Error>(ctx.get_key(&ka)?.clone())
            });
...
async fn memo_cached_executes_on_first_call() {
    ...
    let count = call_count.clone();
    app.update(|ctx| async move {
        let _result: i32 = grepify::memo::cached(&ctx, &"key1", move |_ctx| {
            let count = count.clone();
            async move {
                count.fetch_add(1, Ordering::SeqCst);
                Ok(42)
...
async fn memo_cached_returns_cached_on_second_run() {
    ...
    let count = call_count.clone();
    app.update(|ctx| async move {
        let result: i32 = grepify::memo::cached(&ctx, &"stable_key", move |_ctx| {
            let count = count.clone();
            async move {
                count.fetch_add(1, Ordering::SeqCst);
                Ok(100)
...
async fn update_with_options_full_reprocess_forces_memo_execution() {
    ...
    app.update_with_options(
        grepify::UpdateOptions {
            full_reprocess: true,
            live: false,
            ..grepify::UpdateOptions::default()
        },
        |ctx| async move {
            let result: i32 = ctx
                .memo(&"stable_key", move |_ctx| {
...
async fn memo_cached_reexecutes_on_key_change() {
    ...
    let count = call_count.clone();
    app.update(|ctx| async move {
        let result: i32 = grepify::memo::cached(&ctx, &"v1", move |_ctx| {
            let count = count.clone();
            async move {
                count.fetch_add(1, Ordering::SeqCst);
                Ok(10)
    ...
    let count = call_count.clone();
    app.update(|ctx| async move {
        let result: i32 = grepify::memo::cached(&ctx, &"v2", move |_ctx| {
            let count = count.clone();
            async move {
                count.fetch_add(1, Ordering::SeqCst);
                Ok(20)
...
fn memo_cached_blocking() {
    ...
    let count = call_count.clone();
    app.update_blocking(|ctx| async move {
        let result: String = grepify::memo::cached(&ctx, &("file.rs", 42u64), move |_ctx| {
            let count = count.clone();
            async move {
                count.fetch_add(1, Ordering::SeqCst);
...
fn drop_state_blocking_clears_memoization() {
    ...
    let count = call_count.clone();
    app.update_blocking(|ctx| async move {
        let _: i32 = grepify::memo::cached(&ctx, &"key", move |_ctx| {
            let count = count.clone();
            async move {
                count.fetch_add(1, Ordering::SeqCst);
                Ok(1)
    ...
    let count = call_count.clone();
    app.update_blocking(|ctx| async move {
        let result: i32 = grepify::memo::cached(&ctx, &"key", move |_ctx| {
            let count = count.clone();
            async move {
                count.fetch_add(1, Ordering::SeqCst);
                Ok(2)
...
async fn memo_cached_propagates_closure_error() {
    ...
    let result = app
        .update(|ctx| async move {
            let _: i32 = grepify::memo::cached(&ctx, &"key", |_ctx| async {
                Err(grepify::Error::engine("test error"))
            })
            .await?;
            Ok(())
        })
...
fn update_blocking_propagates_closure_error() {
    ...
    let result = app.update_blocking(|_ctx| async move {
        Err::<(), _>(grepify::Error::engine("sync test error"))
    });
    assert!(result.is_err());
}
...
async fn memo_cached_complex_types() {
    ...
    app.update(|ctx| async move {
        let entities: Vec<Entity> = grepify::memo::cached(&ctx, &"complex_key", |_ctx| async {
            Ok(vec![
                Entity {
                    name: "foo".into(),
                    line: 10,
...
// ---------------------------------------------------------------------------
// #[grepify::function] macro — bare (L0, no memo)
// ---------------------------------------------------------------------------

#[grepify::function]
async fn bare_fn(ctx: &grepify::Ctx, _val: &str) -> grepify::Result<i32> {
    let _ = ctx;
    Ok(1)
}

#[grepify::function]
async fn bare_logic_v1(_ctx: &grepify::Ctx) -> grepify::Result<i32> {
    Ok(1)
}

#[grepify::function]
async fn bare_logic_v2(_ctx: &grepify::Ctx) -> grepify::Result<i32> {
    Ok(2)
}
...
mod batched_test {
    ...
    // A ctx-free `#[grepify::function]` batch impl: gets a logic-hash const and
    // stays directly callable, but takes no `&Ctx`.
    #[grepify::function]
    async fn double_batch(items: Vec<i64>) -> grepify::Result<Vec<i64>> {
    ...
    #[grepify::function]
    async fn triple_batch(items: Vec<i64>) -> grepify::Result<Vec<i64>> {
    ...
    async fn batched_memoizes_per_item() {
        ...
        let batched = Arc::new(grepify::Batched::new(
            double_batch,
...
mod memo_test_basic {
    ...
    async fn the_fn(_ctx: &grepify::Ctx, key: &String) -> grepify::Result<i32> {
...
mod memo_test_borrowed_str {
    ...
    #[grepify::function(memo)]
    async fn len(_ctx: &grepify::Ctx, _input: &str) -> grepify::Result<usize> {
        CALLS.fetch_add(1, Ordering::SeqCst);
        Ok(_input.len())
    }
...
mod memo_test_file_state {
    ...
    use grepify::fs::{FileEntry, FilePath, walk_dir};
    use grepify::{ContextKey, Ctx, FileLike, Result};
...
#[grepify::function(memo)]
async fn hash_fn_a(_ctx: &grepify::Ctx, _key: &String) -> grepify::Result<i32> {
    Ok(111)
}

#[grepify::function(memo)]
async fn hash_fn_b(_ctx: &grepify::Ctx, _key: &String) -> grepify::Result<i32> {
    Ok(222)
}
...
mod function_macro_identity_tests {
    ...
    use grepify::{ContextKey, Ctx, Result};
    ...
    async fn identical_body_functions_do_not_share_memo_entries() {
        let dir = tempfile::tempdir().unwrap();
        let calls = Arc::new(AtomicUsize::new(0));
        let app = grepify::Environment::builder()
            .db_path(dir.path().join("lmdb"))
            .provide_key(calls_key(), calls.clone())
            .build()
            .await
...
mod function_macro_memo_key_tests {
    ...
    #[grepify::function(memo, memo_key(entry = entry_name_version, extra = skip))]
...
async fn public_target_state_api_reconciles_typed_actions_and_tracking_records() {
    ...
    impl TargetHandler<String> for MemoryHandler {
        ...
        ) -> grepify::Result<Option<TargetReconcileOutput<Self::Action, Self::TrackingRecord>>>
        {
        ...
        let (sum, handle) = ctx
            .stats_group("Indexing docs", |group_ctx, _handle| async move {
                let values = group_ctx
                    ...
                    .await?;
                Ok::<_, grepify::Error>(values.into_iter().sum::<i32>())
            })
            .await?;
        assert_eq!(sum, 30);
...
async fn ctx_stats_group_with_options_reports_scoped_child_stats() {
    ...
    app.update(move |ctx| async move {
        let options = grepify::StatsGroupOptions {
            ...
            move |ctx| async move {
                ctx.auto_refresh(&"poller", Duration::from_millis(5), move |_ctx| {
                    ...
                    async move {
                        ...
                        if call == 2 {
                            return Err(grepify::Error::engine("cycle failed"));
                        }
                        if call >= 3 {
...
impl MockApi {
    ...
    async fn call(&self, input: &str) -> grepify::Result<String> {
...
// -- Mode 1: #[grepify::function] (bare — change tracking only) -----------
...
mod mock_bare {
    ...
    #[grepify::function]
    async fn analyze(ctx: &grepify::Ctx, input: &str) -> grepify::Result<String> {
...
mod mock_memo {
    ...
    /// then use bare `#[grepify::function]` + manual `ctx.memo()`.
    ///
...
fn fs_walk_integration() {
    ...
    let files = grepify::fs::walk(dir.path(), &["**/*.rs"]).unwrap();
...
mod mount_macros {
    ...
    async fn run_each(app: &grepify::App, items: Vec<(String, u32)>) -> Vec<u32> {
        app.update(move |ctx| async move {
            let out = grepify::mount_each!(items, |n| each_double(ctx, n)).await?;
            Ok(out)
        })
        .await
        .unwrap()
    ...
    async fn mount_each_reprocesses_only_changed_item() {
        ...
        async fn run(app: &grepify::App, items: Vec<(String, u32)>) {
            app.update(move |ctx| async move {
                grepify::mount_each!(items, |n| sel_double(ctx, n)).await?;
                Ok(())
            })
            .await
            .unwrap();
    ...
    async fn use_mount_skips_unchanged_component_across_runs() {
        ...
        async fn run(app: &grepify::App) -> u32 {
            app.update(|ctx| async move {
                let out = grepify::use_mount!(once_triple(ctx, 7u32)).await?;
                Ok(out)
            })
            .await
            .unwrap()
    ...
    async fn mount_each_accepts_block_form_closure() {
        ...
        let out: Vec<u32> = app
            .update(move |ctx| async move {
                ...
                #[rustfmt::skip]
                let out = grepify::mount_each!(items, |n| { block_id(ctx, n) }).await?;
                Ok(out)
            })
            .await
            .unwrap();
