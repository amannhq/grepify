...
#[grepify::function(logic_tracking = "full")]
async fn full_explicit(_ctx: &Ctx, x: &i64) -> Result<i64> {
    Ok(*x + 2)
}

#[grepify::function(logic_tracking = "self")]
async fn self_only(_ctx: &Ctx, x: &i64) -> Result<i64> {
    Ok(*x + 3)
}

#[grepify::function(logic_tracking = "none")]
...
#[grepify::function(memo, logic_tracking = "none")]
async fn untracked_memo(_ctx: &Ctx, x: &i64) -> Result<i64> {
    Ok(x * 10)
}

// `self` combined with `memo`.
#[grepify::function(memo, logic_tracking = "self")]
...
fn registered_logic_names() -> Vec<&'static str> {
    grepify::COCO_FN_LOGIC.iter().map(|e| e.name).collect()
}
