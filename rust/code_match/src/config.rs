...
use grepify_code_ast::prog_langs::{self, ProgrammingLanguageInfo};
use regex::Regex;
...
pub struct LangConfig {
    ...
    /// [`CodeSource`](grepify_code_ast::CodeSource)'s resolved info).
...
impl LangConfig {
    ...
    /// the `grepify_code_ast` registry, so grammar identity is shared with
