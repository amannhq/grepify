...
use grepify_code_match::{Boundary, Pattern, Prefilter, index_terms, lang};
...
fn index_terms_in_tree_matches_the_parsing_variant() {
    ...
    let mut a = grepify_code_match::index_terms_in_tree(&tree, src, 3);
    let mut b = grepify_code_match::index_terms(src, &cfg, 3);
    a.sort();
    b.sort();
    assert_eq!(a, b);
