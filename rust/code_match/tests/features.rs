...
use grepify_code_match::{Pattern, lang};
...
fn caps_all<'a>(ms: &'a [grepify_code_match::Match], name: &str) -> Vec<&'a str> {
...
fn regex_invalid_errors() {
    ...
    assert!(
        matches!(res, Err(grepify_code_match::Error::Client { .. })),
...
fn non_ascii_never_panics() {
    ...
    for pat in [
        ...
        let _ = grepify_code_match::Pattern::compile(pat, &cfg).map(|p| p.matches("x;"));
    }
}
...
fn non_ascii_sigil() {
    ...
    let cfg = lang::typescript().with_meta_char('§');
    let src = "a = b;";
    let ms = grepify_code_match::Pattern::compile("§A = §B", &cfg)
        .unwrap()
        .matches(src);
    assert!(
        ms.iter()
...
fn overlapping_fragments_are_leftmost_longest_non_overlapping() {
    ...
    let bash = || grepify_code_match::lang::by_name("bash").unwrap();
