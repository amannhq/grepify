...
//! `grepify_code_ast` registry (one tree-sitter version workspace-wide), so
//! one tree is sound for all.

use std::collections::HashMap;
use std::ops::Range;

use grepify_code_ast::{CodeSource, LineIndex, OutputPosition, ParseOutcome, prog_langs};
use grepify_code_match::{Match, Pattern, Prefilter, index_terms_in_tree, lang};
use pyo3::prelude::*;
