...
use grepify_code_ast::view::{SegmentKind, SourceView};
use grepify_code_ast::{CodeSource, prog_langs};
use grepify_ops_text::pattern_matcher::PatternMatcher;
...
pub fn render_ranges(
    ...
    let inner = &src.inner;
    py.detach(|| {
        let ranges: Vec<grepify_code_ast::TextRange> = ranges
            .iter()
            .map(|&(start, end)| grepify_code_ast::TextRange::new(start, end))
            .collect();
        PySourceView::from_view(grepify_code_ast::view::render_ranges(inner, &ranges))
    })
}
