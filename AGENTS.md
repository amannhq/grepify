...
Grepify uses a **declarative** programming model — you specify *what* your output should look like (target states), not *how* to incrementally update it. The engine handles change detection and applies minimal updates automatically.
...
* **Grepify**: declare target states as function of source → engine syncs what changed
...
**Component Path** — Stable identifier for a processing component across runs. Created via `coco.component_subpath("process", filename)`. Grepify uses component paths to:
...
**Target State** — What you want to exist in an external system (a file, a database row, a table). You *declare* target states; Grepify keeps them in sync — creating, updating, or removing as needed.
...
When a processing component finishes, Grepify compares its declared target states with those from the previous run at the same component path:
...
**External modules** (user-facing, e.g. `grepify/ops/sentence_transformers.py`):
...
* Prefix ALL non-public symbols with `_`, including:
  ...
  * Internal package imports: `from grepify.resources import schema as _schema`
* Exception: `TYPE_CHECKING` imports for type hints don't need prefixing

**Internal modules** (e.g. `grepify/_internal/component_ctx.py`, `**/_target.py`):
...
Use `common.create_test_env(__file__)` to create a Grepify `Environment` for tests. It derives a unique `db_path` from the test file path and picks up the current event loop automatically.
...
* Python's `grepify` API is **async-first**: mount APIs (`mount`, `use_mount`, `mount_each`, `map`) are all `async def`; `App.update()`/`App.drop()` are async; sync entry points (`App.update_blocking()`, `App.drop_blocking()`, `start_blocking()`, `stop_blocking()`) are available for CLI and blocking contexts
...
The current codebase is for Grepify v1, which is a fundamental redesign from Grepify v0. Grepify v1 lives on the `main` branch; the legacy v0 code is preserved on the `v0` branch.
...
- For docs diagrams, read `dev/agent-skills/grepify-diagrams/SKILL.md`.
