#!/usr/bin/env python3
"""
Script to generate CLI documentation from Grepify Click commands.

This script uses md-click as the foundation but generates enhanced markdown
documentation that's suitable for inclusion in the Grepify documentation site.
"""
...
import re
import click
from grepify.cli import cli

# Add the grepify python directory to the path
...
def clean_usage_line(usage: str) -> str:
    ...
    # Replace 'cli' with 'grepify' in usage lines and remove 'Usage:' prefix
    cleaned = usage.replace("Usage: cli ", "grepify ")
    # Handle case where it might be "Usage: grepify" already
    if cleaned.startswith("Usage: grepify "):
        cleaned = cleaned.replace("Usage: grepify ", "grepify ")
    return cleaned
...
def main() -> None:
    ...
    print("Generating Grepify CLI documentation...")
