...
# Users can override with GREPIFY_PYTHON if they want a different interpreter.
if [[ -n "${GREPIFY_PYTHON:-}" ]]; then
  PY="$GREPIFY_PYTHON"
  ...
  echo "hint: create/activate a venv (.venv) or set GREPIFY_PYTHON=/path/to/python" >&2
  exit 1
fi
