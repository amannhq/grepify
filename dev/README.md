...
This directory contains development and maintenance scripts for the Grepify project.

## Scripts

### `generate_cli_docs.py`

Automatically generates CLI documentation from the Grepify Click commands.
...
- Extracts help messages from all Click commands in `python/grepify/cli.py`
...
- Runs automatically as a prek hook when `python/grepify/cli.py` is modified
...
- `grepify` package must be importable (the CLI module)
...
- Avoid type-checking Grepify internals by setting `--follow-imports=silent`
- Make Grepify sources discoverable via `MYPYPATH=python`
