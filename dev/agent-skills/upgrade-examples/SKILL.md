...
description: This skill should be used when upgrading the grepify package version in all example pyproject.toml files. It uses regex-based sed commands to efficiently update version constraints across multiple files at once.
---

# Upgrade Example Dependencies

Upgrade the grepify version in all example `pyproject.toml` files.
...
```bash
find examples -name "pyproject.toml" -not -path "*/.venv/*" -exec grep -l "grepify" {} \; | xargs sed -i '' 's/grepify\([^>]*\)>=[0-9][0-9a-zA-Z.]*/grepify\1>=VERSION/g'
```

3. Verify the changes show the correct version:

```bash
grep -r "grepify.*>=" examples --include="pyproject.toml" | grep -v ".venv"
```
...
git commit -m "chore: upgrade examples deps to grepify-VERSION"
...
```bash
gh pr create --base v1 --title "chore: upgrade examples deps to grepify-VERSION" --body ""
```
