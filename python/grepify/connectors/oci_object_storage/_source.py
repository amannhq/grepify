...
:func:`grepify.connectors.kafka.topic_as_stream(...).payloads()`) when
...
try:
    ...
    raise ImportError(
        ...
        "Storage source connector. Please install grepify[oci]."
    ) from e

from grepify._internal.live_component import (
    _IMMEDIATE_READY,
    LiveMapSubscriber,
    LiveStream,
    ReadyAwaitable,
)
from grepify.connectorkits import SingleWatcherGuard
from grepify.resources import file
...
class OCIWalker:
    ...
    async def __aiter__(self) -> AsyncIterator[OCIFile]:
        from grepify.connectorkits.async_adapters import sync_to_async_iter
...
def list_objects(
    ...
    *,
    ...
    docs/src/content/docs/connectors/oci_object_storage.mdx (live bucket watching)
