...
try:
    ...
    raise ImportError(
        "qdrant-client is required to use the Qdrant connector. Please install grepify[qdrant]."
    ) from e
...
from grepify._internal.datatype import TypeChecker
...
class CollectionSchema:
    ...
    Example:
        ```python
        from grepify.connectors.qdrant import CollectionSchema, QdrantVectorDef
        from grepify.resources.schema import VectorSchema
        import numpy as np
...
_collection_provider = coco.register_root_target_states_provider(
    "grepify/qdrant/collection", _CollectionHandler()
)
