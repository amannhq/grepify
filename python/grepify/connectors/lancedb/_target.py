...
try:
    ...
    raise ImportError(
        "lancedb and pyarrow are required to use the LanceDB connector. Please install grepify[lancedb]."
    ) from e
...
from grepify.connectorkits import statediff, target
from grepify.connectorkits.fingerprint import fingerprint_object
from grepify._internal.datatype import (
    AnyType,
    MappingType,
    SequenceType,
    RecordType,
...
from grepify.resources import schema as res_schema
import msgspec

from grepify._internal.context_keys import ContextKey, ContextProvider
...
class LanceType(NamedTuple):
    ...
    from dataclasses import dataclass
    from grepify.connectors.lancedb import LanceType
...
class _RowHandler(coco.TargetHandler[_RowValue, _RowFingerprint]):
    ...
    def _legacy_fingerprint_for_null_backfill(
        ...
        ``None``, the physical row is already correct and only Grepify's
...
_table_provider = coco.register_root_target_states_provider(
    "grepify/lancedb/table", _TableHandler()
)
...
def declare_table_target(
    ...
    *,
    ...
    Args:
        ...
        table_name: Name of the table.
        table_schema: Schema definition including columns and primary key.
        managed_by: Whether the table is managed by "system" (Grepify creates/drops it)
                    or "user" (table must exist, Grepify only manages rows).
