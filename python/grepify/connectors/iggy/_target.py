...
from grepify._internal.target_state import AsyncTargetActionSinkFn
from grepify.connectorkits.fingerprint import fingerprint_bytes, fingerprint_str
...
_topic_provider = coco.register_root_target_states_provider(
    "grepify/iggy/topic", _TopicHandler()
)
...
class IggyTopicTarget(Generic[coco.MaybePendingS], coco.ResolvesTo["IggyTopicTarget"]):
    ...
    The stream and topic are user-managed (Grepify does not create/drop them).
    ...
    def declare_target_state(
        ...
        On commit, Grepify will send a message with the given value
        if the state has changed since the last run.

        Args:
            key: The stable identity used by Grepify.
