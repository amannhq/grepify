...
try:
    ...
    raise ImportError(
        "zvec is required to use the zvec connector. Please install grepify[zvec]."
    ) from e
...
class ZvecType(NamedTuple):
    ...
    import zvec
    from grepify.connectors.zvec import ZvecType
...
class ZvecFtsType(NamedTuple):
    ...
    ```python
    from typing import Annotated
    from grepify.connectors.zvec import ZvecFtsType
...
_collection_provider = coco.register_root_target_states_provider(
    "grepify/zvec/collection", _CollectionHandler()
)
...
def collection_target(
    ...
    *,
    ...
    Args:
        ...
        managed_by: Whether Grepify manages the collection lifecycle
