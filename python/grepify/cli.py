...
from grepify._internal.app import App
from grepify._internal import core as _core
from grepify._internal.environment import (
    Environment,
    LazyEnvironment,
    EnvironmentInfo,
    default_env_lazy,
...
from grepify._internal.setting import get_default_db_path
from grepify.inspect import (
    iter_stable_paths,
...
from grepify._internal.stable_path import StablePath
...
def _setup_logging() -> None:
    ...
    Level is taken from the ``GREPIFY_LOG_LEVEL`` env var (default ``WARNING``).
    Uses ``force=True`` so re-invocation (e.g. tests) replaces any prior config.
    """
    level = os.environ.get("GREPIFY_LOG_LEVEL", "WARNING").upper()
    logging.basicConfig(
...
async def _ls_from_database_async(db_path: str) -> None:
    ...
    try:
        from grepify._internal.setting import Settings
...
def _create_project_files(project_name: str, project_dir: str) -> None:
    """Create project files for a new Grepify project."""

    project_path = pathlib.Path(project_dir)
    project_path.mkdir(parents=True, exist_ok=True)

    # Create main.py
    main_py_content = f'''"""Grepify app template."""
import pathlib
...
def coco_lifespan(builder: coco.EnvironmentBuilder) -> Iterator[None]:
    """Configure the Grepify environment."""
    builder.settings.db_path = pathlib.Path("./grepify.db")
    yield
...
name = "{project_name}"
version = "0.1.0"
description = "A Grepify application"
requires-python = ">=3.11"
dependencies = [
    "grepify>={coco.__version__}",
]
"""
...
A Grepify application.

## Getting Started

Run the app:
```bash
uv run grepify update main.py
```

## Project Structure

- `main.py` - Main application file with your Grepify app definition
    ...
    if app_target:
        ...
        if default_db:
            ...
            raise click.ClickException(
                ...
                "(or set GREPIFY_DB environment variable).\n"
                "  grepify ls ./app.py        # List apps from module\n"
                "  grepify ls --db ./my.db    # List apps from database"
            )
...
def show(
    ...
    if app_target:
        ...
        raise click.ClickException(
            ...
            "  grepify show ./app.py              # from module\n"
            "  grepify show --db ./my.db --app-name MyApp  # from database"
        )
...
@click.option(
    ...
    show_default=True,
    default=False,
    help="Drop existing setup before updating (equivalent to running 'grepify drop' first).",
)
@click.option(
    "--full-reprocess",
    is_flag=True,
...
def update(
    ...
    async def _do(cancelled: Any) -> None:
        from grepify._internal.app import show_progress
        ...
        try:
            ...
            # --reset: drop existing state first (equivalent to `grepify drop ...`)
            if reset:
                if not force:
                    if not _confirm_yes(
...
def init(project_name: str | None, dir: str | None) -> None:
    """
    Initialize a new Grepify project.
    ...
    try:
        ...
        click.echo(f"Created Grepify project '{project_name}' in '{project_dir}'")
        click.echo("\nNext steps:")
        if project_dir != ".":
            click.echo(f"  1. cd {project_dir}")
            click.echo("  2. uv run grepify update main.py")
        else:
            click.echo("  1. uv run grepify update main.py")
    except Exception as e:
