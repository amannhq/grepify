...
class DtypeRegistry:
    """
    Registry for NumPy dtypes used in Grepify.
    Maps NumPy dtypes to their Grepify type kind.
    """
    ...
    def validate_dtype_and_get_kind(cls, dtype: Any) -> str:
        """
        Validate that the given dtype is supported, and get its Grepify kind by dtype.
        """
        if dtype is Any:
            raise TypeError(
...
class LeafType(NamedTuple):
    """
    Any type that is not supported by Grepify.
    """
...
def analyze_type_info(t: Any, *, nullable: bool = False) -> DataTypeInfo:
    """
    Analyze a Python type annotation and extract Grepify-specific type information.
    """
