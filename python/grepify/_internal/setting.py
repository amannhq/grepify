"""
Data types for settings of the grepify library.
"""
...
def get_default_db_path() -> pathlib.Path | None:
    """
    Get the default database path from the GREPIFY_DB environment variable.

    Returns:
        The path from GREPIFY_DB if set, otherwise None.
    """
    db_path = os.getenv("GREPIFY_DB")
...
class Settings:
    """Settings for the grepify library."""
    ...
    def from_env(cls, db_path: os.PathLike[str] | None = None) -> Self:
        ...
        _load_field(
            lmdb_kwargs,
            "max_dbs",
            "GREPIFY_LMDB_MAX_DBS",
            parse=int,
        )
        _load_field(
            lmdb_kwargs,
            "map_size",
            "GREPIFY_LMDB_MAP_SIZE",
            parse=int,
        )
