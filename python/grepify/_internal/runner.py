...
Configure the GPU pool size via the GREPIFY_NUM_GPUS environment variable
(default: 1). Set GREPIFY_RUN_GPU_IN_SUBPROCESS=1 for subprocess isolation.
"""
...
class GPUPool:
    ...
    The default pool size is auto-detected from ``GREPIFY_NUM_GPUS``,
    ...
    def _should_use_subprocess(self) -> bool:
        ...
        if self._use_subprocess is None:
            self._use_subprocess = (
                os.environ.get("GREPIFY_RUN_GPU_IN_SUBPROCESS") == "1"
            )
...
def _warn_subprocess_multi_gpu() -> None:
    ...
    if pool.num_gpus > 1:
        warnings.warn(
            f"GREPIFY_RUN_GPU_IN_SUBPROCESS=1 with num_gpus={pool.num_gpus}: "
