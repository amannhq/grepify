...
rm -rf grepify*.db db1 db2 db_alpha out_* __pycache__
```
...
grepify ls ./no_apps.py

# Should show error: "No apps found after loading..."
grepify update ./no_apps.py
```
...
# Remove one
grepify drop ./multi_app.py:MultiApp1 -f

# List - MultiApp1 should show [+], MultiApp2 should not
grepify ls ./multi_app.py

# Clean up
grepify drop ./multi_app.py:MultiApp2 -f
```
...
grepify ls ./app_not_bound.py

# Update - should work
grepify update ./app_not_bound.py

# Verify target state
ls out_unbound/

# Clean up
grepify drop ./app_not_bound.py -f
```

### 5. Multiple Environments (Different Databases, Same Filename)

Tests apps in different environments are grouped correctly. Both use `grepify.db` but in different directories.

```bash
# List - should show TWO groups (two different db paths, both ending in grepify.db)
grepify ls ./multi_env.py
# Expected output:
#   ./db1/grepify.db:
#     DB1App [+]
#
...
# Update both
grepify update ./multi_env.py:DB1App
grepify update ./multi_env.py:DB2App
...
# Clean up
grepify drop ./multi_env.py:DB1App -f
grepify drop ./multi_env.py:DB2App -f
rm -rf db1 db2
```
...
grepify ls ./same_name_diff_env.py
# Expected output:
#   alpha (./db_alpha/grepify.db):
#     MyApp [+]
#
#   default (./grepify.db):
#     MyApp [+]
...
grepify update ./same_name_diff_env.py:MyApp
# Expected error: "Multiple apps named 'MyApp' found in different environments..."

# Update with @env_name specifier
grepify update ./same_name_diff_env.py:MyApp@alpha
grepify update ./same_name_diff_env.py:MyApp@default
...
grepify drop ./same_name_diff_env.py:MyApp@alpha -f

# List - alpha should show [+], default should not
grepify ls ./same_name_diff_env.py

# Clean up
grepify drop ./same_name_diff_env.py:MyApp@default -f
rm -rf db_alpha
```
...
grepify update ./single_app.py:NonExistent
```
...
grepify update ./app1.py

# List using --db option (no module needed)
grepify ls --db ./grepify.db
# Expected: Shows TestApp1

# Clean up
grepify drop ./app1.py -f
```
