...
def run_cli(
    *args: str,
    ...
    """Run a grepify CLI command and return the result."""
    cmd = ["grepify", *args]
...
class TestListFromDatabase:
    ...
    def test_ls_db_shows_persisted_apps(self) -> None:
        ...
        result = run_cli("ls", "--db", "./grepify.db")
...
# =============================================================================
# Test: Default DB path from GREPIFY_DB environment variable
# =============================================================================


class TestDefaultDbPath:
    """Tests for the default db path from GREPIFY_DB environment variable."""

    def test_ls_uses_default_db_from_env(self) -> None:
        """grepify ls without args should use GREPIFY_DB if set."""
        ...
        shutil.copytree(TEST_DIR / "grepify.db", db_path)

        # Now run ls without args but with GREPIFY_DB set
        env = os.environ.copy()
        env["GREPIFY_DB"] = str(db_path)
        cmd = ["grepify", "ls"]
        result = subprocess.run(
            cmd,
            cwd=TEST_DIR,
    ...
    def test_ls_without_args_errors_when_no_env_var(self) -> None:
        """grepify ls without args should error when GREPIFY_DB is not set."""
        # Ensure GREPIFY_DB is not set
        env = os.environ.copy()
        env.pop("GREPIFY_DB", None)
        cmd = ["grepify", "ls"]
        result = subprocess.run(
            cmd,
            cwd=TEST_DIR,
        ...
        assert result.returncode != 0
        assert "GREPIFY_DB" in result.stderr

    def test_update_app_with_default_db_from_env(self) -> None:
        """grepify update should work when app uses GREPIFY_DB for db_path."""
        db_path = TEST_DIR / "default_db_test.db"

        # Set GREPIFY_DB and run update
        env = os.environ.copy()
        env["GREPIFY_DB"] = str(db_path)
        cmd = ["grepify", "update", "./app_default_db.py"]
        result = subprocess.run(
            cmd,
            cwd=TEST_DIR,
            capture_output=True,
...
class TestInitCommand:
    """Tests for the grepify init command."""

    def test_init_creates_project_structure(self) -> None:
        """grepify init MyProject should create basic project files."""
