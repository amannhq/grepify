...
from grepify.ops.litellm import LiteLLMEmbedder  # noqa: E402
from grepify.resources.embedder import Embedder  # noqa: E402
...
async def test_litellm_embedder_single_text_api() -> None:
    ...
    with patch(
        "grepify.ops.litellm.litellm.aembedding",
...
async def test_litellm_embedder_retries_transient_embedding_errors() -> None:
    ...
    with (
        patch("grepify.ops.litellm.litellm.aembedding", new=mocked_embedding),
        patch("grepify.ops.litellm._asyncio.sleep", new=AsyncMock()) as sleep,
    ):
        vec = await embedder.embed("hello")
