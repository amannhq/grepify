...
from grepify._internal import runner as _runner_mod
from grepify._internal.runner import (
    GPUPool,
    GPURunner,
    configure_gpu_pool,
    current_gpu,
...
def test_detect_num_gpus_explicit_env_zero(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GREPIFY_NUM_GPUS", "0")
    monkeypatch.delenv("CUDA_VISIBLE_DEVICES", raising=False)
    assert _detect_num_gpus() == 1
...
def test_detect_num_gpus_explicit_env_overrides_cuda_visible(
    ...
    monkeypatch.setenv("GREPIFY_NUM_GPUS", "2")
