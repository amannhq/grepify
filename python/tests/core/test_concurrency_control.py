...
def test_env_var_fallback() -> None:
    """GREPIFY_MAX_INFLIGHT_COMPONENTS env var is used when AppConfig omits it."""
    _tracker.reset()
    original = os.environ.get("GREPIFY_MAX_INFLIGHT_COMPONENTS")
    try:
        os.environ["GREPIFY_MAX_INFLIGHT_COMPONENTS"] = "2"
        app = coco.App(
        ...
        if original is None:
            os.environ.pop("GREPIFY_MAX_INFLIGHT_COMPONENTS", None)
        else:
            os.environ["GREPIFY_MAX_INFLIGHT_COMPONENTS"] = original
