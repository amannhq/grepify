...
from grepify._internal.setting import LmdbSettings, Settings
...
def test_engine_wire_format_is_flat() -> None:
    ...
    db_path = pathlib.Path("/tmp/grepify_test")
    settings = Settings(
        db_path=db_path,
...
def test_from_env_reads_lmdb_env_vars(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GREPIFY_LMDB_MAX_DBS", "512")
    monkeypatch.setenv("GREPIFY_LMDB_MAP_SIZE", str(2 * 1024 * 1024 * 1024))
    settings = Settings.from_env(db_path=pathlib.Path("/tmp/grepify_test"))
    assert settings.db_settings.max_dbs == 512
    assert settings.db_settings.map_size == 2 * 1024 * 1024 * 1024


def test_from_env_falls_back_to_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("GREPIFY_LMDB_MAX_DBS", raising=False)
    monkeypatch.delenv("GREPIFY_LMDB_MAP_SIZE", raising=False)
    settings = Settings.from_env(db_path=pathlib.Path("/tmp/grepify_test"))
    assert settings.db_settings.max_dbs == 1024
    assert settings.db_settings.map_size == 0x1_0000_0000
