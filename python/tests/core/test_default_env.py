...
from grepify._internal.environment import reset_default_env_for_tests
...
def _default_env_from_env_var() -> Iterator[None]:
    """
    Fixture that sets GREPIFY_DB env var and uses a lifespan that does NOT
    set db_path explicitly.
    """
    # Reset any previously initialized default environment
    reset_default_env_for_tests()

    old_env = os.environ.get("GREPIFY_DB")
    os.environ["GREPIFY_DB"] = str(_env_db_path_from_env_var)

    try:
        # Lifespan that does NOT set db_path - relies on GREPIFY_DB env variable
        @coco.lifespan
        ...
        if old_env is not None:
            os.environ["GREPIFY_DB"] = old_env
        else:
            os.environ.pop("GREPIFY_DB", None)
...
@pytest.mark.asyncio
async def test_default_env_uses_grepify_db_env_var(
    _default_env_from_env_var: None,
) -> None:
    """Test that default env uses GREPIFY_DB when lifespan doesn't set db_path."""
...
def test_app_uses_grepify_db_env_var(_default_env_from_env_var: None) -> None:
    """Test that app works when using GREPIFY_DB env var for db_path."""
    app = coco.App(
