---
name: grepify
description: This skill should be used when building data processing pipelines with Grepify, a Python library for incremental data transformation. Use when the task involves processing files/data into databases, creating vector embeddings, building knowledge graphs, ETL workflows, or any data pipeline requiring automatic change detection and incremental updates. Grepify is Python-native (supports any Python types), has no DSL, and uses version 1.0.0 or later.
---

# Grepify

Grepify is a Python library for building incremental data processing pipelines with declarative target states. Think spreadsheets or React for data pipelines: declare what the output should look like based on current input, and Grepify automatically handles incremental updates, change detection, and syncing to external systems.

## Overview

Grepify enables building data pipelines that:
...
This creates: `main.py`, `pyproject.toml`, `README.md`. The generated `main.py` sets the database location in its lifespan via `builder.settings.db_path = pathlib.Path("./grepify.db")`.

### Add Dependencies

```toml
# For vector embeddings with PostgreSQL
dependencies = ["grepify>=1.0.0", "sentence-transformers", "asyncpg"]

# For LLM extraction
dependencies = ["grepify>=1.0.0", "litellm", "instructor", "pydantic>=2.0"]
```
...
The `@coco.fn` decorator marks functions as Grepify processing functions. Add `memo=True` to skip re-execution when inputs/code are unchanged:
...
**Declare** what should exist -- Grepify handles creation/update/deletion:
...
The `@coco.lifespan` decorator registers the function to the default Grepify environment, which is shared among all apps by default. `ContextKey` also serves as the stable identity for sources/targets -- the `key` string must remain stable across runs.
...
```python
from grepify.resources.id import generate_id, IdGenerator
...
import grepify as coco
from grepify.connectors import localfs, postgres
from grepify.ops.text import RecursiveSplitter
from grepify.ops.sentence_transformers import SentenceTransformerEmbedder
from grepify.resources.chunk import Chunk
from grepify.resources.file import FileLike, PatternFilePathMatcher
from grepify.resources.id import IdGenerator

DATABASE_URL = "postgres://grepify:grepify@localhost/grepify"
...
Grepify provides connectors for reading from and writing to external systems:
...
```bash
grepify init my-project              # Create new project
grepify update main.py               # Run app
grepify update main.py:my_app        # Run specific app
grepify update main.py -L            # Run in live mode (continuous)
grepify update main.py --full-reprocess  # Reprocess everything
grepify drop main.py [-f]            # Drop and reset all state
grepify ls [main.py]                 # List apps
grepify show main.py [--tree]        # Show component paths
```
...
[`examples/`](https://github.com/grepify/grepify/tree/main/examples) — start
from the one closest to the task and adapt it. Each has its own `README.md` and
most Python examples have a `.env.example`; see
[`examples/AGENTS.md`](https://github.com/grepify/grepify/blob/main/examples/AGENTS.md)
for the full map, credentials, and per-example run commands. Good starting points:

- Vector search → `text_embedding` (Postgres)
- Code search → `code_embedding`
- Custom transform → `files_transform`

### External

- Grepify documentation lives in-repo under `docs/src/content/docs/`
- [GitHub Examples](https://github.com/grepify/grepify/tree/main/examples)

## Version Note

This skill is for Grepify `>=1.0.0` (v1). It uses a completely different API from v0.
...
|---|---|
| `@grepify.flow_def`, `FlowBuilder`, `Flow`, `open_flow` | `coco.App` + a `@coco.fn` main function |
...
| `grepify.sources.LocalFile`, `grepify.sources.*` | connector APIs, e.g. `localfs.walk_dir(...)` |
| `grepify.functions.SplitRecursively`, `grepify.functions.*` | `grepify.ops.*`, e.g. `RecursiveSplitter` |
| `grepify.targets.Postgres`, `grepify.targets.*` / `storages.*` | connector targets, e.g. `postgres.declare_table_target(...)` |
| `transform_flow`, `grepify.op.function()` | plain `@coco.fn` functions |
| `grepify.init()`, `settings`, `GREPIFY_DATABASE_URL` | `coco.App(coco.AppConfig(...))`; state lives in a local db path |
| CLI `grepify setup` | no setup step — just `grepify update` (`-L`/`--live` for live mode) |
