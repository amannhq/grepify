# Grepify API Reference

Quick reference for the most commonly used Grepify APIs.
...
All APIs live directly on the `coco` module. There is no separate `grepify.asyncio` module.

---

## `@coco.fn` Decorator

Mark a function as a Grepify processing function.
...
```bash
grepify init [PROJECT_NAME]              # Create new project
grepify update APP_TARGET                # Run app once
grepify update APP_TARGET -L             # Run in live mode
grepify update APP_TARGET --full-reprocess  # Force reprocess all
grepify update APP_TARGET --reset        # Reset state before running
grepify drop APP_TARGET [-f]             # Drop app and all state
grepify ls [APP_TARGET] [--db PATH]      # List apps
grepify show APP_TARGET [--tree]         # Show component paths
```
...
**Import:** `from grepify.ops.text import ...`
...
**Import:** `from grepify.ops.sentence_transformers import SentenceTransformerEmbedder`
...
**Import:** `from grepify.resources.file import ...`
...
**Import:** `from grepify.resources.id import ...`
...
**Import:** `from grepify.resources.schema import VectorSchema`
