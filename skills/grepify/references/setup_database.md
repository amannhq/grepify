...
Setup instructions for databases used with Grepify.
...
services:
  postgres:
    ...
    container_name: grepify-postgres
    environment:
      POSTGRES_USER: grepify
      POSTGRES_PASSWORD: grepify
      POSTGRES_DB: grepify
    ports:
      - "5432:5432"
    volumes:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U grepify"]
      interval: 5s
      timeout: 5s
      retries: 5
...
Connection URL: `postgres://grepify:grepify@localhost:5432/grepify`
...
```bash
# .env
POSTGRES_URL=postgres://grepify:grepify@localhost:5432/grepify
```
...
docker logs grepify-qdrant      # Check logs
```
