...
|------|--------|---------------------|
| Walk files | `localfs.walk_dir` | `grepify::walk` |
| Per-file incremental skip | `@coco.fn(memo=True)` | `#[grepify::function(memo)]` |
| Language detection | `detect_code_language` | `grepify_ops_text::prog_langs::detect_language` |
| Chunking | `RecursiveSplitter` | `grepify_ops_text::split::RecursiveChunker` |
...
| Vector store | `postgres.TableTarget` + `declare_vector_index` | `grepify::postgres` `TableTarget` + `declare_vector_index` |
| Stable row ids | `IdGenerator.next_id(chunk.text)` | `IdGenerator::next_id(ctx, chunk_text)` |
| Query | pgvector `<=>` | pgvector `<=>` |

Chunking and language detection *do* exist in the engine (`grepify_ops_text`) but aren't re-exported by the `grepify` SDK crate, so we depend on that crate directly.

## Prerequisites

- **Postgres with the `pgvector` extension.** Quick start:
  ```sh
  docker run -d --name grepify-pg -p 5432:5432 \
    -e POSTGRES_USER=grepify -e POSTGRES_PASSWORD=grepify -e POSTGRES_DB=grepify \
    pgvector/pgvector:pg16
  ```
  Override the connection with `POSTGRES_URL` (default `postgres://grepify:grepify@localhost/grepify`).
