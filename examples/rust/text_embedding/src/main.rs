...
//!   - source           : `grepify::fs::walk` (cf. `localfs.walk_dir`)
//!   - per-file compute  : `#[grepify::function(memo)]` (cf. `@coco.fn(memo=True)`)
//!   - chunking          : `grepify::ops::text::RecursiveSplitter` (cf. `RecursiveSplitter`)
//!   - embeddings        : `grepify::ops::sentence_transformers` all-MiniLM-L6-v2
//!   - target            : `postgres::TableTarget` + pgvector index

use std::path::PathBuf;
use std::sync::LazyLock;

use grepify::ops::sentence_transformers::SentenceTransformerEmbedder;
use grepify::ops::text::{RecursiveChunkConfig, RecursiveSplitter};
use grepify::postgres;
use grepify::prelude::*;
use sqlx::Row;
...
fn database_url() -> String {
    std::env::var("POSTGRES_URL")
        .unwrap_or_else(|_| "postgres://grepify:grepify@localhost/grepify".to_string())
}
    ...
    match args.first().map(String::as_str) {
        ...
        sub => {
            ...
            let app = Environment::builder()
                .db_path(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(".grepify_db"))
                .provide_key(&DB, db)
