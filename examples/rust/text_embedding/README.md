...
| ---------------- | ---------------------------------------- | -------------------------------------------------- |
| Source           | `localfs.walk_dir`                       | `grepify::fs::walk`                              |
| Per-file compute | `@coco.fn(memo=True) process_file`       | `#[grepify::function(memo)] process_file`         |
| Chunking         | `RecursiveSplitter` (markdown)           | `grepify_ops_text` `RecursiveChunker` (markdown)  |
...
```bash
export POSTGRES_URL=postgres://grepify:grepify@localhost/grepify   # pgvector-enabled
